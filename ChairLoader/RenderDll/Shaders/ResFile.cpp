...
auto FComputeLowercaseOv0 = PreyFunction<unsigned(const char* const szData)>(0x18057F0);
...
static_assert(sizeof(CResFile) == 208);
static_assert(sizeof(CResFileLookupDataMan) == 72);

namespace
{
PreyGlobal<CryCriticalSection> g_cResLock(0x2B31108);
PreyGlobal<CryCriticalSection> g_cAsyncResLock(0x2B31130);
}
...
void CResFile::Tick()
{
	...
	CResFile* pRF, * pNext;
	for (pRF = m_RootStream->m_PrevStream; pRF != m_RootStream.Get(); pRF = pNext)
	{
...
int CResFile::mfLoadDir(SResStreamInfo* pStreamInfo)
{
	...
	if (pStreamInfo)
	{
		...
		if (nSizeDir)
		{
			...
			CryAutoLock<CryCriticalSection> lock(pStreamInfo->m_StreamLock);
			pStreamInfo->m_dirReadStreams.push_back(gEnv->pSystem->GetStreamEngine()->StartRead(
...
void CResStreamCallback::StreamAsyncOnComplete(IReadStream* pStream, unsigned nError)
{
	...
	}
	{
		...
		{
			CryWarning("mfGetOpenEntry() returned NULL, possibly because r_shadersAllowCompilation=1 and r_shadersAsyncActivation=1. Try r_shadersAsyncActivation=0 in your user.cfg.");
		}
	}
}
...
int CResFile::mfFileRead(SDirEntry* de)
{
	...
	if (m_pStreamInfo)
	{
		...
		pEntry->m_readStream = gEnv->pSystem->GetStreamEngine()->StartRead(eStreamTaskTypeShader, m_name.c_str(), &m_pStreamInfo->m_Callback, &StrParams);
		return -1;
	}
