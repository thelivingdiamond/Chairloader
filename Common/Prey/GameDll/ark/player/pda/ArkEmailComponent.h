...
public:
	...
	std::unordered_map<uint64_t, bool> m_downloadHidden;
	std::unordered_map<uint64_t, bool> m_sentEmails;
	std::vector<uint64_t> m_emailsThatResetOnCharacterChange;
	std::vector<uint64_t> m_emailsThatPersistThroughSimReset;
	...
	static inline auto FInit = PreyFunction<void(ArkEmailComponent* const _this)>(0x16D7E30);
	static inline auto FGiveData = PreyFunction<void(ArkEmailComponent* const _this, const uint64_t _id)>(0x16D7C20);
	static inline auto FGiveDownload = PreyFunction<void(ArkEmailComponent* const _this, const uint64_t _downloadId)>(0x16D7DD0);
	static inline auto FHasDownload = PreyFunction<bool(const ArkEmailComponent* const _this, const uint64_t _id)>(0x16D6750);
	static inline auto FSerialize = PreyFunction<void(ArkEmailComponent* const _this, TSerialize _ser)>(0x16D83C0);
	static inline auto FSetEmailSent = PreyFunction<void(ArkEmailComponent* const _this, const uint64_t _id, bool _bSent)>(0x16D84F0);
	static inline auto FIsEmailSent = PreyFunction<bool(const ArkEmailComponent* const _this, uint64_t _id)>(0x16D8010);
	static inline auto FFormatStringWithFrom = PreyFunction<bool(wstring& _input, const uint64_t _id)>(0x16D77B0);
	static inline auto FGetContentForUI = PreyFunction<ArkEmailComponent::SEmailUIContent(uint64_t _emailId)>(0x16D7980);
	static inline auto FResetSimulation = PreyFunction<void(ArkEmailComponent* const _this)>(0x16D81A0);
	static inline auto FResetForCharacterChange = PreyFunction<void(ArkEmailComponent* const _this)>(0x16D80E0);
	static inline auto FIsDownloadHidden = PreyFunction<bool(const ArkEmailComponent* const _this, uint64_t _id)>(0x16D7EA0);
	static inline auto FSetDownloadHidden = PreyFunction<void(ArkEmailComponent* const _this, uint64_t _id, bool _bHidden)>(0x16D8460);
	static inline auto FIsValidData = PreyFunction<bool(const ArkEmailComponent* const _this, const uint64_t _id)>(0x16D80A0);
};
#endif // !MOONCRASH
