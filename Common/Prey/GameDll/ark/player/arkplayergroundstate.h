...
public:
	...
	ArkSimpleTimer m_stumbleSwitchTimer;
	ArkSimpleTimer m_stumbleTimer;
	string m_deathPoseRun;
	string m_deathPoseCrawl;
	string m_deathPoseSneak;
	EStance m_requestedStance;
	int m_sprintZoomHandle;
	float m_leanPitchLimit;
	float m_leanYawLimit;
	float m_leanCancelDefaultSpeedSq;
	float m_leanCancelStandSpeedSq;
	float m_preLandJumpWindow;
	float m_turnTravelAngle;
	float m_turnTravelSpeed;
	float m_stumbleXAdd;
	...
	static inline auto FExit = PreyFunction<void(ArkPlayerMovementStates::Ground* const _this)>(0x12D6230);
	static inline auto FLimitRotation = PreyFunction<void(const ArkPlayerMovementStates::Ground* const _this, const ArkPlayerMovementFSM& _fsm, Quat& _rotation)>(0x12D67C0);
	static inline auto FLoadConfig = PreyFunction<void(ArkPlayerMovementStates::Ground* const _this, const XmlNodeRef& _root)>(0x12D6CC0);
	static inline auto FUpdate = PreyFunction<void(ArkPlayerMovementStates::Ground* const _this, ArkPlayerMovementFSM& _fsm, const float _frameTime)>(0x12D7390);
	static inline auto FOnSneakButtonPressed = PreyFunction<bool(ArkPlayerMovementStates::Ground* const _this, ArkPlayerMovementFSM& _fsm)>(0x12D71D0);
	static inline auto FShouldBeSprinting = PreyFunction<bool(const ArkPlayerMovementFSM& _fsm)>(0x12D72C0);
	static inline auto FGetAdditionalMovementOnGoo = PreyFunction<Vec3*(ArkPlayerMovementStates::Ground* const _this, Vec3* _return_value_, const float _movementLength)>(0x12D62B0);
	static inline auto FGetLeanInput = PreyFunction<float(const ArkPlayerMovementStates::Ground* const _this, const ArkPlayerMovementFSM& _fsm)>(0x12D6790);
	static inline auto FGetMovementInput = PreyFunction<Vec2*(const ArkPlayerMovementStates::Ground* const _this, Vec2* _return_value_, const ArkPlayer& _player)>(0x12DC890);
	static inline auto FGetDeathPose = PreyFunction<const char* (const ArkPlayerMovementStates::Ground* const _this)>(0x12D6730);
	static inline auto FOnGroundProbeCompleteFront = PreyFunction<bool(ArkPlayerMovementStates::Ground* const _this, const RayCastResult& _result)>(0x12D7170);
	static inline auto FOnGroundProbeCompleteBack = PreyFunction<bool(ArkPlayerMovementStates::Ground* const _this, const RayCastResult& _result)>(0x12D7150);
	static inline auto FOnGroundProbeCompleteLeft = PreyFunction<bool(ArkPlayerMovementStates::Ground* const _this, const RayCastResult& _result)>(0x12D7190);
	static inline auto FOnGroundProbeCompleteRight = PreyFunction<bool(ArkPlayerMovementStates::Ground* const _this, const RayCastResult& _result)>(0x12D71B0);
};
