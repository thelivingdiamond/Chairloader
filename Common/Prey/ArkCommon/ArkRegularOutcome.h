...
// ArkRegularOutcome
// Header:  Prey/ArkCommon/ArkRegularOutcome.h
class ArkRegularOutcome
{ // Size=4 (0x4)
public:
	using ResultBits = std::bitset<320000>;
	using DataType = unsigned;
...
#if 0
	ArkRegularOutcome(float _arg0_);
	void SetInverted(bool _arg0_) const;
	void SetChanceIndex(uint64_t _arg0_) const;
	void SetSampleIndex(uint64_t _arg0_) const;
	void SetResultIndex(uint64_t _arg0_) const;
	bool GetInverted() const;
	uint64_t GetChanceIndex() const;
	uint64_t GetSampleIndex() const;
	uint64_t GetResultIndex() const;
