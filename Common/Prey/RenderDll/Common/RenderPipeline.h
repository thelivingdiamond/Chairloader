...
#include <CryEngine/crycommon/crythreadsaferenderercontainer.h>
#include <CryEngine/crycommon/tarray.h>
#include <Prey/Cry3DEngine/SVO/SceneTree.cpp>
...
#include <_unknown/CThreadSafeRendererContainer.h>
#include <_unknown/CThreadSafeWorkerContainer.h>
#include <_unknown/TArray.h>
#include <_unknown/UVertStreamPtr.h>
...
// Header:  CryEngine/renderdll/common/renderpipeline.h
// Include: Prey/RenderDll/Common/RenderPipeline.h
struct SCompareRendItemZPass
{ // Size=1 (0x1)
	bool operator()(const SRendItem& a, const SRendItem& b) const { return FoperatorRBLRBR(this, a, b); }

	static inline auto FoperatorRBLRBR = PreyFunction<bool(const SCompareRendItemZPass* const _this, const SRendItem& a, const SRendItem& b)>(0xEC7170);
};
...
struct SRenderPipeline
{ // Size=14592 (0x3900)
	// SRenderPipeline::SPipelineStreamInfo
	// Header:  CryEngine/renderdll/common/renderpipeline.h
	struct SPipelineStreamInfo
	{ // Size=16 (0x10)
		ID3D11Buffer* pStream;
		int nOffset;
		int nStride;

	#if 0
		SPipelineStreamInfo();
		SPipelineStreamInfo(ID3D11Buffer* _arg0_, int _arg1_, int _arg2_);
	#endif
	};
	...
	SThreadInfo m_TI[2];
	SThreadInfo m_OldTI[1];
	CThreadSafeRendererContainer<Color_tpl<float> > m_fogVolumeContibutions[2];
	unsigned m_PersFlags1;
	unsigned m_PersFlags2;
	uint8_t m_nSceneMask;
	int m_FlagsPerFlush;
	...
	SRenderPipeline();
	~SRenderPipeline();
	void OnDemandVertexDeclaration(SOnDemandD3DVertexDeclaration& out, const int nStreamMask, const int vertexformat, const bool bMorph, const bool bInstanced) { FOnDemandVertexDeclaration(this, out, nStreamMask, vertexformat, bMorph, bInstanced); }
	void InitD3DVertexDeclarations() { FInitD3DVertexDeclarations(this); }
	EVertexFormat AddD3DVertexDeclaration(uint64_t numDescs, const D3D11_INPUT_ELEMENT_DESC* inputLayout) { return FAddD3DVertexDeclaration(this, numDescs, inputLayout); }

#if 0
	CRenderView* RenderView() const;
	SShaderTechnique* GetStartTechnique() const;
	void InitWaveTables();
	EVertexFormat MaxD3DVertexDeclaration();
#endif

	static inline auto FSRenderPipelineOv1 = PreyFunction<void(SRenderPipeline* const _this)>(0xFD8590);
	static inline auto FBitNotSRenderPipeline = PreyFunction<void(SRenderPipeline* const _this)>(0xFD8B70);
	static inline auto FOnDemandVertexDeclaration = PreyFunction<void(SRenderPipeline* const _this, SOnDemandD3DVertexDeclaration& out, const int nStreamMask, const int vertexformat, const bool bMorph, const bool bInstanced)>(0xF31520);
	static inline auto FInitD3DVertexDeclarations = PreyFunction<void(SRenderPipeline* const _this)>(0xF30550);
	static inline auto FAddD3DVertexDeclaration = PreyFunction<EVertexFormat(SRenderPipeline* const _this, uint64_t numDescs, const D3D11_INPUT_ELEMENT_DESC* inputLayout)>(0xF27600);
};
#endif // !MOONCRASH
