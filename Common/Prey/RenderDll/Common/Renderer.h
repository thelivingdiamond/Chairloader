...
#include <CryEngine/crycommon/crythreadimpl_win32.h>
#include <CryEngine/crycommon/ijobmanager.h>
#include <Prey/CryCore/Platform/Linux_Win32Wrapper.h>
...
#include <_unknown/Array.h>
#include <_unknown/CryLockT.h>
#include <_unknown/IRenderMesh.h>
#include <_unknown/N3DEngineCommon__SCausticInfo.h>
#include <_unknown/N3DEngineCommon__SVisAreaInfo.h>
...
class CWater;
enum EDataType;
enum class EDeviceMemoryHeap;
enum class EDeviceMemoryType;
...
// CSkinningDataPool
// Header:  CryEngine/renderdll/common/Renderer.h
// Include: Prey/RenderDll/Common/Renderer.h
class CSkinningDataPool
{ // Size=40 (0x28)
public:
	// CSkinningDataPool::SPage
	// Header:  CryEngine/renderdll/common/Renderer.h
	struct SPage
	{ // Size=8 (0x8)
		CSkinningDataPool::SPage* pNext;
	};

	uint8_t* m_pPool;
	CSkinningDataPool::SPage* m_pPages;
	uint64_t m_nPoolSize;
	uint64_t m_nPoolUsed;
	uint64_t m_nPageAllocated;

	uint8_t* Allocate(uint64_t nBytes) { return FAllocate(this, nBytes); }
	...
	static inline auto FAllocate = PreyFunction<uint8_t* (CSkinningDataPool* const _this, uint64_t nBytes)>(0xFCEA60);
};
...
struct S3DEngineCommon
{ // Size=160 (0xA0)
	enum class EVisAreaFlags
	{
		VAF_EXISTS_FOR_POSITION = 1,
		VAF_CONNECTED_TO_OUTDOOR = 2,
		VAF_AFFECTED_BY_OUT_LIGHTS = 4,
		VAF_MASK = 7,
	};
	...
	void Update(unsigned long nThreadID) { FUpdate(this, nThreadID); }

	static inline auto FUpdate = PreyFunction<void(S3DEngineCommon* const _this, unsigned long nThreadID)>(0xFD7870);
};
...
public:
	...
	enum class #unnamed_enum_nMeshPoolMaxTimeoutCounter
	{
		nMeshPoolMaxTimeoutCounter = 150,
	};

	using ShadowFrustumListsCache = std::map<uint64_t, PodArray<unsigned short, 0>*>;
	using TListRendererEventListeners = std::list<IRendererEventListener *,std::allocator<IRendererEventListener *> >;

	volatile bool m_bStopRendererAtFrameEnd;
	Matrix44 m_IdentityMatrix;
	Matrix44 m_ViewMatrix;
	Matrix44 m_CameraMatrix;
	Matrix44 m_CameraZeroMatrix;
	Matrix44 m_ProjMatrix;
	Matrix44 m_TranspOrigCameraProjMatrix;
	Matrix44 m_CameraProjMatrix;
	Matrix44 m_CameraProjZeroMatrix;
	Matrix44 m_InvCameraProjMatrix;
	Matrix44 m_TempMatrices[4][8];
	Matrix44 m_CameraMatrixPrev[7];
	Matrix44 m_CameraProjMatrixPrev;
	Matrix44 m_CameraProjMatrixPrevAvg;
	Matrix44 m_CameraMatrixNearest;
	Matrix44 m_CameraMatrixNearestPrev;
	Vec2 m_vProjMatrixSubPixoffset;
	Vec3 m_vSegmentedWorldOffset;
	uint8_t m_bDeviceLost;
	uint8_t m_bSystemResourcesInit;
	...
	CTextureManager* m_pTextureManager;
	ArkBinkManager* m_pBinkManager;
	ArkHighlightSystem* m_pHighlightSystem;
	float m_fPrevTime;
	uint8_t m_nUseZpass : 2;
	int m_dwHDRCropWidth;
	int m_dwHDRCropHeight;
	S3DEngineCommon m_p3DEngineCommon;
	std::map<uint64_t, PodArray<unsigned short, 0>*> m_FrustumsCache;
	ShadowFrustumMGPUCache m_ShadowFrustumMGPUCache;
	CShaderMan m_cEF;
	_smart_ptr<IMaterial> m_pDefaultMaterial;
	_smart_ptr<IMaterial> m_pTerrainDefaultMaterial;
	...
	IFFont* m_pDefaultFont;
	CryCriticalSection m_renderObjectAccessLock;
	static inline auto m_iGeomInstancingThreshold = PreyGlobal<int>(0x2CA02D0);
	static constexpr const int nMaxParticleContainer = 8192;
	int m_nCREParticleCount[2];
	JobManager::SJobState m_ComputeVerticesJobState;
	CFillRateManager m_FillRateManager;
	...
	virtual void ResumeDevice();
	virtual void SyncComputeVerticesJobs();
	virtual void DrawStringW(IFFont_RenderProxy* pFont, float x, float y, float z, const wchar_t* pStr, const bool asciiMultiLine, const STextDrawContext& ctx) const;
	virtual void RT_DrawStringW(IFFont_RenderProxy* pFont, float x, float y, float z, const wchar_t* pStr, const bool asciiMultiLine, const STextDrawContext& ctx) const = 0;
	...
	virtual void RT_BeginFrame() = 0;
	virtual void RT_EndFrame() = 0;
	virtual void RT_BeginLoadingFrame(unsigned _frameId) = 0;
	virtual void RT_EndLoadingFrame(unsigned _frameId) = 0;
	virtual void RT_SwitchToNativeResolutionBackbuffer(bool resolveBackBuffer) = 0;
	virtual void RT_Init() = 0;
	virtual void RT_ShutDown(unsigned nFlags) = 0;
	virtual bool RT_CreateDevice() = 0;
	virtual void RT_Reset() = 0;
	virtual void RT_FlushTextMessages();
	virtual void RT_SetCull(int mode) = 0;
	virtual void RT_SetScissor(bool bEnable, int sX, int sY, int sWdt, int sHgt) = 0;
	virtual void RT_RenderScene(CRenderView* pRenderView, int nFlags, SThreadInfo& TI, void (*RenderFunc)()) = 0;
	virtual void RT_SetCameraInfo() = 0;
	virtual void RT_CreateResource(SResourceAsync* pRes) = 0;
	virtual void RT_ReleaseResource(SResourceAsync* pRes) = 0;
	virtual void RT_ReleaseRenderResources(unsigned nFlags) = 0;
	virtual void RT_UnbindResources() = 0;
	virtual void RT_UnbindTMUs() = 0;
	virtual void RT_CreateRenderResources() = 0;
	virtual void RT_PrecacheDefaultShaders() = 0;
	...
	virtual void RT_FlashRenderPlaybackLockless(IFlashPlayer_RenderProxy* pPlayer, int cbIdx, bool finalPlayback);
	virtual void RT_FlashRemoveTexture(ITexture* pTexture);
	virtual void RT_ReleaseCB(void* pVCB) = 0;
	virtual void RT_ReleaseRS(std::shared_ptr<CDeviceResourceSet>& pRS) = 0;
	virtual void RT_DrawDynVB(SVF_P3F_C4B_T2F* pBuf, uint16_t* pInds, unsigned nVerts, unsigned nInds, const PublicRenderPrimitiveType nPrimType) = 0;
	virtual void RT_Draw2dImage(float xpos, float ypos, float w, float h, CTexture* pTexture, float s0, float t0, float s1, float t1, float angle, unsigned long col, float z) = 0;
	virtual void RT_Draw2dImageStretchMode(bool bStretch) = 0;
	virtual void RT_Push2dImage(float xpos, float ypos, float w, float h, CTexture* pTexture, float s0, float t0, float s1, float t1, float angle, unsigned long col, float z) = 0;
	virtual void RT_Draw2dImageList() = 0;
	virtual void RT_DrawImageWithUV(float xpos, float ypos, float z, float w, float h, int texture_id, float* s, float* t, unsigned long col, bool filtered) = 0;
	virtual void RT_SetViewport(int x, int y, int width, int height, int id) = 0;
	virtual void RT_ClearTarget(CTexture* pTex, const ColorF& color) = 0;
	virtual void RT_RenderDebug(bool bRenderStats) = 0;
	virtual void RT_SetRendererCVar(ICVar* pCVar, const char* pArgText, const bool bSilentMode) = 0;
	void RT_PostLevelLoading() { FRT_PostLevelLoading(this); }
	void RT_DisableTemporalEffects() { FRT_DisableTemporalEffects(this); }
	virtual void FlashRenderInternal(IFlashPlayer_RenderProxy* pPlayer, bool bDoRealRender) = 0;
	virtual void FlashRenderPlaybackLocklessInternal(IFlashPlayer_RenderProxy* pPlayer, int cbIdx, bool bFinalPlayback, bool bDoRealRender) = 0;
	virtual bool FlushRTCommands(bool bWait, bool bImmediatelly, bool bForce);
	virtual bool ForceFlushRTCommands();
	virtual void WaitForParticleBuffer() = 0;
	virtual void RequestFlushAllPendingTextureStreamingJobs(int nFrames);
	virtual void SetTexturesStreamingGlobalMipFactor(float fFactor);
	virtual void EF_ClearTargetsImmediately(unsigned nFlags) = 0;
	virtual void EF_ClearTargetsImmediately(unsigned nFlags, const ColorF& Colors, float fDepth, uint8_t nStencil) = 0;
	virtual void EF_ClearTargetsImmediately(unsigned nFlags, const ColorF& Colors) = 0;
	virtual void EF_ClearTargetsImmediately(unsigned nFlags, float fDepth, uint8_t nStencil) = 0;
	virtual void EF_ClearTargetsLater(unsigned nFlags) = 0;
	virtual void EF_ClearTargetsLater(unsigned nFlags, const ColorF& Colors, float fDepth, uint8_t nStencil) = 0;
	virtual void EF_ClearTargetsLater(unsigned nFlags, const ColorF& Colors) = 0;
	virtual void EF_ClearTargetsLater(unsigned nFlags, float fDepth, uint8_t nStencil) = 0;
	virtual void AddListener(IRendererEventListener* pRendererEventListener);
	virtual void RemoveListener(IRendererEventListener* pRendererEventListener);
	virtual ERenderType GetRenderType() const;
	virtual void* Init(int x, int y, int width, int height, unsigned cbpp, int zbpp, int sbits, EFullscreenMode _fullscreenMode, void* hinst, void* Glhwnd, bool bReInit, const SCustomRenderInitArgs* pCustomArgs, bool bShaderCacheGen) = 0;
	virtual void* GetCurrentContextHWND();
	virtual bool IsCurrentContextMainVP();
	virtual int GetFeatures();
	virtual int GetNumGeomInstances();
	...
	virtual int GetCurrentNumberOfDrawCalls(const unsigned EFSListMask);
	virtual float GetCurrentDrawCallRTTimes(const unsigned EFSListMask);
	virtual bool ChangeResolution(int nNewWidth, int nNewHeight, int nNewColDepth, int nNewRefreshHZ, EFullscreenMode _fullscreenMode, bool bForceReset) = 0;
	virtual bool CheckDeviceLost();
	virtual EScreenAspectRatio GetScreenAspect(int nWidth, int nHeight);
	virtual Vec2 SetViewportDownscale(float xscale, float yscale);
	virtual void Release();
	...
	virtual int GetWhiteTextureId() const;
	virtual void PrintToScreen(float x, float y, float size, const char* buf);
	virtual void TextToScreen(float x, float y, const char* format, ... buffer);
	virtual void TextToScreenColor(int x, int y, float r, float g, float b, float a, const char* format, ... buffer);
	virtual IArkBinkManager* GetIArkBinkManager();
	virtual IArkHighlightSystem* GetIArkHighlightSystem();
	void WriteXY(int x, int y, float xscale, float yscale, float r, float g, float b, float a, const char* format, ... buffer) { FWriteXY(this, x, y, xscale, yscale, r, g, b, a, format, buffer); }
	virtual void Draw2dText(float posX, float posY, const char* pStr, const SDrawTextInfo& ti);
	virtual void Draw2dTextWithDepth(float posX, float posY, float posZ, const char* pStr, const SDrawTextInfo& ti);
	virtual void DrawTextQueued(Vec3 pos, SDrawTextInfo& ti, const char* text);
	virtual void DrawTextQueued(Vec3 pos, SDrawTextInfo& ti, const char* format, char* args);
	virtual void PrintResourcesLeaks() = 0;
	virtual float ScaleCoordX(float value) const;
	...
	virtual int GetWireframeMode();
	virtual CRenderView* GetRenderViewForThread(int nThreadID, bool bRecursive);
	Matrix44 GetCameraMatrix() { alignas(Matrix44) std::byte _return_buf_[sizeof(Matrix44)]; return *FGetCameraMatrix(this, reinterpret_cast<Matrix44*>(_return_buf_)); }
	const Matrix44& GetPreviousFrameCameraMatrix() const { return FGetPreviousFrameCameraMatrix(this); }
	void SetPreviousFrameCameraMatrix(const Matrix44& m) { FSetPreviousFrameCameraMatrix(this, m); }
	virtual void GetPolyCount(int& nPolygons, int& nShadowPolys);
	virtual int GetPolyCount();
	virtual void GetMemoryUsage(ICrySizer* pSizer);
	virtual void GetBandwidthStats(float* fBandwidthRequested);
	...
	virtual int GetStencilBpp();
	virtual void FlushPendingTextureTasks();
	virtual void ForceTrimTextureStreamer();
	virtual void SetShadowJittering(float shadowJittering);
	virtual float GetShadowJittering() const;
	virtual void* FX_AllocateCharInstCB(SSkinningData* __unnamed1, unsigned __unnamed2);
	virtual void FX_ClearCharInstCB(unsigned __unnamed1);
	virtual EShaderQuality EF_GetShaderQuality(EShaderType eST);
	virtual ERenderQuality EF_GetRenderQuality() const;
	void RefreshSystemShaders() { FRefreshSystemShaders(this); }
	unsigned EF_BatchFlags(SShaderItem& SH, CRenderObject* pObj, CRendElementBase* re, const SRenderingPassInfo& passInfo, int nAboveWater) { return FEF_BatchFlags(this, SH, pObj, re, passInfo, nAboveWater); }
	virtual void FX_PipelineShutdown(bool bFastShutdown) = 0;
	virtual void RT_GraphicsPipelineShutdown() = 0;
	virtual IOpticsElementBase* CreateOptics(EFlareType type) const;
	virtual ISoftOcclusionQuery* CreateSoftOcclusionQuery() const;
	virtual bool EF_PrecacheResource(IShader* pSH, float fMipFactor, float fTimeToReady, int Flags);
	virtual bool EF_PrecacheResource(IRenderMesh* _pPB, IMaterial* pMaterial, float fMipFactor, float fTimeToReady, int nFlags, int nUpdateId);
	virtual bool EF_PrecacheResource(CDLight* pLS, float fMipFactor, float fTimeToReady, int Flags, int nUpdateId);
	virtual CRenderObject* EF_AddPolygonToScene(SShaderItem& si, int numPts, const SVF_P3F_C4B_T2F* verts, const SPipTangents* tangs, CRenderObject* obj, const SRenderingPassInfo& passInfo, uint16_t* inds, int ninds, int nAW);
	virtual CRenderObject* EF_AddPolygonToScene(SShaderItem& si, CRenderObject* obj, const SRenderingPassInfo& passInfo, int numPts, int ninds, SVF_P3F_C4B_T2F* & verts, SPipTangents* & tangs, uint16_t* & inds, int nAW);
	void FX_CheckOverflow(int nVerts, int nInds, CRendElementBase* re, int* nNewVerts, int* nNewInds) { FFX_CheckOverflow(this, nVerts, nInds, re, nNewVerts, nNewInds); }
	void FX_Start(CShader* ef, int nTech, CShaderResources* Res, CRendElementBase* re) { FFX_Start(this, ef, nTech, Res, re); }
	virtual void EF_AddMultipleParticlesToScene(const SAddParticlesToSceneJob* jobs, uint64_t numJobs, const SRenderingPassInfo& passInfo);
	virtual void EF_SetShaderMissCallback(void (*callback)(const char*));
	virtual const char* EF_GetShaderMissLogPath();
	virtual string* EF_GetShaderNames(int& nNumShaders);
	virtual IShader* EF_LoadShader(const char* name, int flags, uint64_t nMaskGen);
	virtual SShaderItem EF_LoadShaderItem(const char* szName, bool bShare, int flags, SInputShaderResources* Res, uint64_t nMaskGen, const IRenderer::SLoadShaderItemArgs* pArgs);
	virtual uint64_t EF_GetRemapedShaderMaskGen(const char* name, uint64_t nMaskGen, bool bFixup);
	virtual uint64_t EF_GetShaderGlobalMaskGenFromString(const char* szShaderName, const char* szShaderGen, uint64_t nMaskGen);
	virtual const char* EF_GetStringFromShaderGlobalMaskGen(const char* szShaderName, uint64_t nMaskGen);
	virtual bool EF_ReloadFile(const char* szFileName);
	...
	virtual CRenderObject* EF_GetObject_Temp(int nThreadID);
	virtual CRenderObject* EF_DuplicateRO(CRenderObject* pSrc, const SRenderingPassInfo& passInfo);
	virtual CRenderObject* EF_GetObject();
	virtual void EF_FreeObject(CRenderObject* pObj);
	virtual void EF_AddEf(CRendElementBase* re, SShaderItem& pSH, CRenderObject* obj, const SRenderingPassInfo& passInfo, int nList, int nAW);
	virtual void EF_InvokeShadowMapRenderJobs(CRenderView* pRenderView, const int nFlags);
	...
	virtual int EF_AddDeferredLight(const CDLight& pLight, float fMult, const SRenderingPassInfo& passInfo, const SRendParams& _rendParams);
	virtual void EF_ReleaseDeferredData();
	virtual SInputShaderResources* EF_CreateInputShaderResource(IRenderShaderResources* pOptionalCopyFrom);
	virtual void ClearPerFrameData(const SRenderingPassInfo& passInfo);
	virtual ColorF EF_GetLightAnimColor(uint8_t _style, uint8_t _phase, float _animSpeed);
	virtual bool EF_UpdateDLight(SRenderLight* dl);
	void EF_CheckLightMaterial(CDLight* pLight, uint16_t nRenderLightID, const SRenderingPassInfo& passInfo, const SRendParams& _rendParams) { FEF_CheckLightMaterial(this, pLight, nRenderLightID, passInfo, _rendParams); }
	virtual void EF_AddWaterSimHit(const Vec3& vPos, const float scale, const float strength);
	virtual void EF_DrawWaterSimHits();
	virtual void EF_QueryImpl(ERenderQueryTypes eQuery, void* pInOut0, unsigned nInOutSize0, void* pInOut1, unsigned nInOutSize1);
	virtual void FX_SetState(int st, int AlphaRef, int RestoreState) = 0;
	void FX_SetStencilState(int st, unsigned nStencRef, unsigned nStencMask, unsigned nStencWriteMask, bool bForceFullReadMask) { FFX_SetStencilState(this, st, nStencRef, nStencMask, nStencWriteMask, bForceFullReadMask); }
	virtual uint64_t EF_PostEffectParamHandleFromName(const char* _szParamName) const;
	virtual void EF_SetPostEffectParam(const SSetPostEffectParam& _param) const;
	virtual void EF_SetPostEffectParam(const SSetPostEffectParamVec4& _param) const;
	virtual void EF_SetPostEffectParam(const SSetPostEffectParamString& _param) const;
	virtual float EF_GetPostEffectParam(uint64_t _handle) const;
	virtual Vec4 EF_GetPostEffectParamVec4(uint64_t _handle) const;
	virtual const char* EF_GetPostEffectParamString(uint64_t _handle) const;
	virtual bool EF_IsPostEffectParam(const char* _szParamName) const;
	...
	virtual void ForceGC();
	virtual _smart_ptr<IRenderMesh> CreateRenderMesh(const char* szType, const char* szSourceName, IRenderMesh::SInitParamerers* pInitParams, ERenderMeshType eBufType);
	virtual _smart_ptr<IRenderMesh> CreateRenderMeshInitialized(const void* pVertBuffer, int nVertCount, EVertexFormat eVF, const uint16_t* pIndices, int nIndices, const PublicRenderPrimitiveType nPrimetiveType, const char* szType, const char* szSourceName, ERenderMeshType eBufType, int nMatInfoCount, int nClientTextureBindID, bool (*PrepareBufferCallback)(IRenderMesh*, bool), void* CustomData, bool bOnlyVideoBuffer, bool bPrecache, const SPipTangents* pTangents, bool bLockForThreadAcc, Vec3* pNormals);
	virtual int GetMaxActiveTexturesARB();
	virtual void GetThreadIDs(unsigned long& mainThreadID, unsigned long& renderThreadID) const;
	virtual void SF_ConfigMask(IRenderer::ESFMaskOp maskOp, unsigned stencilRef);
	virtual int SF_CreateTexture(int width, int height, int numMips, uint8_t* pData, ETEX_Format eTF, int flags);
	...
	virtual ITexture* CreateTexture(const char* name, int width, int height, int numMips, uint8_t* pData, ETEX_Format eTF, int flags);
	virtual void SetProfileMarker(const char* label, CRenderer::ESPM mode) const;
	virtual uint16_t PushFogVolumeContribution(const ColorF& fogVolumeContrib, const SRenderingPassInfo& passInfo);
	void GetFogVolumeContribution(uint16_t idx, ColorF& rColor) const { FGetFogVolumeContribution(this, idx, rColor); }
	virtual int GetMaxTextureSize();
	virtual ShadowFrustumMGPUCache* GetShadowFrustumMGPUCache();
	virtual const StaticArray<int,16,int>& GetCachedShadowsResolution() const;
	virtual void SetCachedShadowsResolution(const StaticArray<int,16,int>& arrResolutions);
	virtual void UpdateCachedShadowsLodCount(int nGsmLods) const;
	virtual void ExecuteAsyncDIP();
	alloc_info_struct* GetFreeChunk(int bytes_count, int nBufSize, PodArray<alloc_info_struct,0>& alloc_info, const char* szSource) { return FGetFreeChunk(this, bytes_count, nBufSize, alloc_info, szSource); }
	bool ReleaseChunk(int p, PodArray<alloc_info_struct,0>& alloc_info) { return FReleaseChunk(this, p, alloc_info); }
	...
	virtual _smart_ptr<IGraphicsDeviceConstantBuffer> CreateGraphiceDeviceConstantBuffer();
	virtual void MakeMatrix(const Vec3& pos, const Vec3& angles, const Vec3& scale, Matrix34* mat);
	void SyncMainWithRender() { FSyncMainWithRender(this); }
	void UpdateConstParamsPF(const SRenderingPassInfo& passInfo) { FUpdateConstParamsPF(this, passInfo); }
	void RT_UpdateConstParamsPF(CRenderView* _pRenderView) { FRT_UpdateConstParamsPF(this, _pRenderView); }
	virtual unsigned GetActiveGPUCount() const;
	void Logv(const char* format, ... _arg1_) { FLogv(this, format, _arg1_); }
	void RT_UpdateLightVolumes() { FRT_UpdateLightVolumes(this); }
	void RT_SetSkinningPoolId(unsigned poolId) { FRT_SetSkinningPoolId(this, poolId); }
	void EF_AddEf_NotVirtual(CRendElementBase* re, SShaderItem& SH, CRenderObject* obj, const SRenderingPassInfo& passInfo, int nList, int nAW) { FEF_AddEf_NotVirtual(this, re, SH, obj, passInfo, nList, nAW); }
	void FX_StartMerging() { FFX_StartMerging(this); }
	void FX_PushVP() { FFX_PushVP(this); }
	void FX_PopVP() { FFX_PopVP(this); }
	void EF_AddRTStat(CTexture* pTex, int nFlags, int nW, int nH) { FEF_AddRTStat(this, pTex, nFlags, nW, nH); }
	...
	unsigned FX_GetMSAAMode() const { return FFX_GetMSAAMode(this); }
	void FX_SetMSAAFlagsRT() { FFX_SetMSAAFlagsRT(this); }
	void FX_ApplyShaderQuality(const EShaderType eST) { FFX_ApplyShaderQuality(this, eST); }
	bool IsHDRModeEnabled() const { return FIsHDRModeEnabled(this); }
	virtual const CCamera& GetCamera();
	const CRenderCamera& GetRCamera() { return FGetRCamera(this); }
	static int GetTexturesStreamPoolSize() { return FGetTexturesStreamPoolSize(); }
	void EF_RemoveParticlesFromScene() { FEF_RemoveParticlesFromScene(this); }
	void PrepareParticleRenderObjects(Array<SAddParticlesToSceneJob const ,int,NArray::ArrayStorage> aJobs, int nREStart, SRenderingPassInfo passInfoOriginal) { FPrepareParticleRenderObjects(this, aJobs, nREStart, passInfoOriginal); }
	bool EF_GetParticleListAndBatchFlags(unsigned& nBatchFlags, int& nList, SShaderItem& shaderItem, CRenderObject* pRO, const SRenderingPassInfo& passInfo) { return FEF_GetParticleListAndBatchFlags(this, nBatchFlags, nList, shaderItem, pRO, passInfo); }
	void FreePermanentRenderObjects(int bufferId) { FFreePermanentRenderObjects(this, bufferId); }
	virtual bool StopRendererAtFrameEnd(unsigned timeoutMilliseconds);
	virtual void ResumeRendererFromFrameEnd();
	virtual void SetTexturePrecaching(bool stat);
	...
	CTextureManager* GetTextureManager();
	ArkBinkManager* GetArkBinkManager();
	ArkHighlightSystem* GetArkHighlightSystem();
	float ScaleCoordXInternal(float _arg0_) const;
	float ScaleCoordYInternal(float _arg0_) const;
	void ScaleCoordInternal(float& _arg0_, float& _arg1_) const;
	void SetWidth(int _arg0_);
	void SetHeight(int _arg0_);
	void SetPixelAspectRatio(float _arg0_);
	int GetBackbufferWidth();
	int GetBackbufferHeight();
	int RT_GetPolyCount();
	int RT_GetCurrGpuID() const;
	int GetDepthBpp();
	void EF_AddClientPolys(const SRenderingPassInfo& _arg0_);
	void EF_RemovePolysFromScene();
	bool FX_TryToMerge(CRenderObject* _arg0_, CRenderObject* _arg1_, CRendElementBase* _arg2_, bool _arg3_);
	const SSF_GlobalDrawParams* SF_GetGlobalDrawParams() const;
	void ClearSkinningDataPool();
	void LogStrv(char* _arg0_, ... _arg1_);
	void LogShv(char* _arg0_, ... _arg1_);
	void Log(char* _arg0_);
	void RT_SetLightVolumeShaderFlags();
	void EF_TransformDLights();
	void EF_IdentityDLights();
	void EF_PrintRTStats(const char* _arg0_);
	int FX_ApplyShadowQuality();
	void FX_ResetMSAAFlagsRT();
	bool IsShadowPassEnabled() const;
	const bool IsEditorMode() const;
	const bool IsShaderCacheGenMode() const;
	float GetMipDistFactor();
	void EF_AddParticle(CREParticle* _arg0_, SShaderItem& _arg1_, CRenderObject* _arg2_, const SRenderingPassInfo& _arg3_);
	static void* operator new(uint64_t _arg0_);
	static void operator delete(void* _arg0_);
	void SetTextureAlphaChannelFromRGB(uint8_t* _arg0_, int _arg1_);
	bool IsHdrOutputEnabled() const;
	EDisplayOutputMode GetPixelOutputMode() const;
	ETEX_Format GetPixelOutputFormat() const;
#endif

	static inline auto FCRenderer = PreyFunction<void(CRenderer* const _this)>(0xFCCF40);
	static inline auto FBitNotCRenderer = PreyFunction<void(CRenderer* const _this)>(0xFCDB40);
	static inline auto FInitRenderer = PreyFunction<void(CRenderer* const _this)>(0xFD5730);
	static inline auto FPostInit = PreyFunction<void(CRenderer* const _this)>(0xFD66E0);
	static inline auto FCreateDeviceMemoryHeap = PreyFunction<bool(CRenderer* const _this, EDeviceMemoryHeap _heapId, EDeviceMemoryType _type, uint64_t _size, uint64_t _bankSize, uint64_t _reserveSize)>(0x1A302A0);
	static inline auto FStartRenderIntroMovies = PreyFunction<void(CRenderer* const _this, bool skipArkIntro)>(0xFD7420);
	static inline auto FStopRenderIntroMovies = PreyFunction<void(CRenderer* const _this, bool bWaitForFinished)>(0xFD7480);
	static inline auto FIsRenderingIntroMovies = PreyFunction<bool(const CRenderer* const _this)>(0xFD6620);
	static inline auto FPostLevelLoading = PreyFunction<void(CRenderer* const _this)>(0xFD67C0);
	static inline auto FPreShutDown = PreyFunction<void(CRenderer* const _this)>(0xFD6850);
	static inline auto FPostShutDown = PreyFunction<void(CRenderer* const _this)>(0xFD6820);
	static inline auto FSuspendDevice = PreyFunction<void(CRenderer* const _this)>(0xFD7580);
	static inline auto FResumeDevice = PreyFunction<void(CRenderer* const _this)>(0xFD7580);
	static inline auto FSyncComputeVerticesJobs = PreyFunction<void(CRenderer* const _this)>(0xFF1F40);
	static inline auto FDrawStringW = PreyFunction<void(const CRenderer* const _this, IFFont_RenderProxy* pFont, float x, float y, float z, const wchar_t* pStr, const bool asciiMultiLine, const STextDrawContext& ctx)>(0xFCF3F0);
	static inline auto FRT_CurThreadList = PreyFunction<int(CRenderer* const _this)>(0xFD6950);
	static inline auto FRT_FlushTextMessages = PreyFunction<void(CRenderer* const _this)>(0xFD69B0);
	static inline auto FRT_CreateREPostProcess = PreyFunction<void(CRenderer* const _this, CRendElementBase* * re)>(0xFD6910);
	static inline auto FRT_FlashRender = PreyFunction<void(CRenderer* const _this, IFlashPlayer_RenderProxy* pPlayer)>(0xEE51A0);
	static inline auto FRT_FlashRenderPlaybackLockless = PreyFunction<void(CRenderer* const _this, IFlashPlayer_RenderProxy* pPlayer, int cbIdx, bool finalPlayback)>(0xEE51B0);
	static inline auto FRT_FlashRemoveTexture = PreyFunction<void(CRenderer* const _this, ITexture* pTexture)>(0xEE5190);
	static inline auto FRT_PostLevelLoading = PreyFunction<void(CRenderer* const _this)>(0xFD6A10);
	static inline auto FRT_DisableTemporalEffects = PreyFunction<void(CRenderer* const _this)>(0xFD6990);
	static inline auto FFlushRTCommands = PreyFunction<bool(CRenderer* const _this, bool bWait, bool bImmediatelly, bool bForce)>(0xFD9410);
	static inline auto FForceFlushRTCommands = PreyFunction<bool(CRenderer* const _this)>(0xFD94B0);
	static inline auto FRequestFlushAllPendingTextureStreamingJobs = PreyFunction<void(CRenderer* const _this, int nFrames)>(0xF6EFC0);
	static inline auto FSetTexturesStreamingGlobalMipFactor = PreyFunction<void(CRenderer* const _this, float fFactor)>(0xF71170);
	static inline auto FAddListener = PreyFunction<void(CRenderer* const _this, IRendererEventListener* pRendererEventListener)>(0xFCE9C0);
	static inline auto FRemoveListener = PreyFunction<void(CRenderer* const _this, IRendererEventListener* pRendererEventListener)>(0xFD6CE0);
	static inline auto FGetRenderType = PreyFunction<ERenderType(const CRenderer* const _this)>(0x4FD7F0);
	static inline auto FGetCurrentContextHWND = PreyFunction<void* (CRenderer* const _this)>(0xFD4F70);
	static inline auto FIsCurrentContextMainVP = PreyFunction<bool(CRenderer* const _this)>(0x1A302A0);
	static inline auto FGetFeatures = PreyFunction<int(CRenderer* const _this)>(0xF40D40);
	static inline auto FGetNumGeomInstances = PreyFunction<int(CRenderer* const _this)>(0x1CBB0B0);
	static inline auto FGetNumGeomInstanceDrawCalls = PreyFunction<int(CRenderer* const _this)>(0x1CBB0B0);
	static inline auto FGetCurrentNumberOfDrawCallsOv2 = PreyFunction<int(CRenderer* const _this)>(0x1CBB0B0);
	static inline auto FGetCurrentNumberOfDrawCallsOv1 = PreyFunction<void(CRenderer* const _this, int& nGeneral, int& nShadowGen)>(0x1333E90);
	static inline auto FGetCurrentNumberOfDrawCallsOv0 = PreyFunction<int(CRenderer* const _this, const unsigned EFSListMask)>(0x1CBB0B0);
	static inline auto FGetCurrentDrawCallRTTimes = PreyFunction<float(CRenderer* const _this, const unsigned EFSListMask)>(0x714650);
	static inline auto FCheckDeviceLost = PreyFunction<bool(CRenderer* const _this)>(0x13B0900);
	static inline auto FGetScreenAspect = PreyFunction<EScreenAspectRatio(CRenderer* const _this, int nWidth, int nHeight)>(0xFD5320);
	static inline auto FSetViewportDownscale = PreyFunction<Vec2*(CRenderer* const _this, Vec2* _return_value_, float xscale, float yscale)>(0xFD7240);
	static inline auto FRelease = PreyFunction<void(CRenderer* const _this)>(0xFD6BB0);
	static inline auto FFreeResources = PreyFunction<void(CRenderer* const _this, int nFlags)>(0xFD3DB0);
	static inline auto FInitSystemResources = PreyFunction<void(CRenderer* const _this, int nFlags)>(0xFD6500);
	static inline auto FSetState = PreyFunction<void(CRenderer* const _this, int State, int AlphaRef)>(0xFD7220);
	static inline auto FPushWireframeMode = PreyFunction<void(CRenderer* const _this, int mode)>(0xFD6900);
	static inline auto FPopWireframeMode = PreyFunction<void(CRenderer* const _this)>(0xFD66D0);
	static inline auto FSaveTga = PreyFunction<bool(const CRenderer* const _this, uint8_t* sourcedata, int sourceformat, int w, int h, const char* filename, bool flip)>(0xFD6E00);
	static inline auto FGetWhiteTextureId = PreyFunction<int(const CRenderer* const _this)>(0xFD5480);
	static inline auto FPrintToScreen = PreyFunction<void(CRenderer* const _this, float x, float y, float size, const char* buf)>(0xFD68A0);
	static inline auto FTextToScreen = PreyFunction<void(CRenderer* const _this, float x, float y, const char* format, ... buffer)>(0xFD7650);
	static inline auto FTextToScreenColor = PreyFunction<void(CRenderer* const _this, int x, int y, float r, float g, float b, float a, const char* format, ... buffer)>(0xFD7750);
	static inline auto FGetIArkBinkManager = PreyFunction<IArkBinkManager* (CRenderer* const _this)>(0xFD5080);
	static inline auto FGetIArkHighlightSystem = PreyFunction<IArkHighlightSystem* (CRenderer* const _this)>(0xFD5090);
	static inline auto FWriteXY = PreyFunction<void(CRenderer* const _this, int x, int y, float xscale, float yscale, float r, float g, float b, float a, const char* format, ... buffer)>(0xFD7A70);
	static inline auto FDraw2dText = PreyFunction<void(CRenderer* const _this, float posX, float posY, const char* pStr, const SDrawTextInfo& ti)>(0xFCF0A0);
	static inline auto FDraw2dTextWithDepth = PreyFunction<void(CRenderer* const _this, float posX, float posY, float posZ, const char* pStr, const SDrawTextInfo& ti)>(0xFCF0D0);
	static inline auto FDrawTextQueuedOv1 = PreyFunction<void(CRenderer* const _this, Vec3 pos, SDrawTextInfo& ti, const char* text)>(0xFCF410);
	static inline auto FDrawTextQueuedOv0 = PreyFunction<void(CRenderer* const _this, Vec3 pos, SDrawTextInfo& ti, const char* format, char* args)>(0xFCF500);
	static inline auto FScaleCoordX = PreyFunction<float(const CRenderer* const _this, float value)>(0xF263A0);
	static inline auto FScaleCoordY = PreyFunction<float(const CRenderer* const _this, float value)>(0xF263C0);
	static inline auto FScaleCoord = PreyFunction<void(const CRenderer* const _this, float& x, float& y)>(0xF6F390);
	static inline auto FGetWidth = PreyFunction<int(CRenderer* const _this)>(0xEB4A40);
	static inline auto FGetHeight = PreyFunction<int(CRenderer* const _this)>(0xEB4A30);
	static inline auto FGetPixelAspectRatio = PreyFunction<float(const CRenderer* const _this)>(0xF6B330);
	static inline auto FGetOverlayWidth = PreyFunction<int(CRenderer* const _this)>(0xF40DE0);
	static inline auto FGetOverlayHeight = PreyFunction<int(CRenderer* const _this)>(0xF40DD0);
	static inline auto FGetNearestRangeMax = PreyFunction<float(const CRenderer* const _this)>(0xF6ADC0);
	static inline auto FGetWireframeMode = PreyFunction<int(CRenderer* const _this)>(0xF304F0);
	static inline auto FGetRenderViewForThread = PreyFunction<CRenderView* (CRenderer* const _this, int nThreadID, bool bRecursive)>(0xFD5300);
	static inline auto FGetCameraMatrix = PreyFunction<Matrix44*(CRenderer* const _this, Matrix44* _return_value_)>(0xFD46B0);
	static inline auto FGetPreviousFrameCameraMatrix = PreyFunction<const Matrix44& (const CRenderer* const _this)>(0xFD5240);
	static inline auto FSetPreviousFrameCameraMatrix = PreyFunction<void(CRenderer* const _this, const Matrix44& m)>(0xFD7150);
	static inline auto FGetPolyCountOv1 = PreyFunction<void(CRenderer* const _this, int& nPolygons, int& nShadowPolys)>(0x1333E90);
	static inline auto FGetPolyCountOv0 = PreyFunction<int(CRenderer* const _this)>(0x1CBB0B0);
	static inline auto FGetMemoryUsage = PreyFunction<void(CRenderer* const _this, ICrySizer* pSizer)>(0xFD50C0);
	static inline auto FGetBandwidthStats = PreyFunction<void(CRenderer* const _this, float* fBandwidthRequested)>(0x1333E90);
	static inline auto FSetTextureStreamListener = PreyFunction<void(CRenderer* const _this, ITextureStreamListener* pListener)>(0x1333E90);
	static inline auto FGetFrameID = PreyFunction<int(CRenderer* const _this, bool bIncludeRecursiveCalls)>(0xFD4F80);
	static inline auto FFlushTextMessagesOv1 = PreyFunction<void(CRenderer* const _this)>(0xFD3B50);
	static inline auto FFlushTextMessagesOv0 = PreyFunction<void(CRenderer* const _this, CTextMessages& messages, bool reset)>(0xFD35B0);
	static inline auto FGetStencilBpp = PreyFunction<int(CRenderer* const _this)>(0xF6B590);
	static inline auto FFlushPendingTextureTasks = PreyFunction<void(CRenderer* const _this)>(0xFD3570);
	static inline auto FForceTrimTextureStreamer = PreyFunction<void(CRenderer* const _this)>(0xFD3BA0);
	static inline auto FSetShadowJittering = PreyFunction<void(CRenderer* const _this, float shadowJittering)>(0xFD7210);
	static inline auto FGetShadowJittering = PreyFunction<float(const CRenderer* const _this)>(0xFD5390);
	static inline auto FFX_AllocateCharInstCB = PreyFunction<void* (CRenderer* const _this, SSkinningData* __unnamed1, unsigned __unnamed2)>(0x1CBB0B0);
	static inline auto FFX_ClearCharInstCB = PreyFunction<void(CRenderer* const _this, unsigned __unnamed1)>(0x1333E90);
	static inline auto FEF_GetShaderQuality = PreyFunction<EShaderQuality(CRenderer* const _this, EShaderType eST)>(0xFD0720);
	static inline auto FEF_GetRenderQuality = PreyFunction<ERenderQuality(const CRenderer* const _this)>(0xFD0670);
	static inline auto FRefreshSystemShaders = PreyFunction<void(CRenderer* const _this)>(0xF36950);
	static inline auto FEF_BatchFlags = PreyFunction<unsigned(CRenderer* const _this, SShaderItem& SH, CRenderObject* pObj, CRendElementBase* re, const SRenderingPassInfo& passInfo, int nAboveWater)>(0xEC7D00);
	static inline auto FCreateOptics = PreyFunction<IOpticsElementBase* (const CRenderer* const _this, EFlareType type)>(0xFCEC00);
	static inline auto FCreateSoftOcclusionQuery = PreyFunction<ISoftOcclusionQuery* (const CRenderer* const _this)>(0xEB3A30);
	static inline auto FEF_PrecacheResourceOv2 = PreyFunction<bool(CRenderer* const _this, IShader* pSH, float fMipFactor, float fTimeToReady, int Flags)>(0x1A302A0);
	static inline auto FEF_PrecacheResourceOv1 = PreyFunction<bool(CRenderer* const _this, IRenderMesh* _pPB, IMaterial* pMaterial, float fMipFactor, float fTimeToReady, int nFlags, int nUpdateId)>(0xFD0CD0);
	static inline auto FEF_PrecacheResourceOv0 = PreyFunction<bool(CRenderer* const _this, CDLight* pLS, float fMipFactor, float fTimeToReady, int Flags, int nUpdateId)>(0xFD0E90);
	static inline auto FEF_AddPolygonToSceneOv1 = PreyFunction<CRenderObject* (CRenderer* const _this, SShaderItem& si, int numPts, const SVF_P3F_C4B_T2F* verts, const SPipTangents* tangs, CRenderObject* obj, const SRenderingPassInfo& passInfo, uint16_t* inds, int ninds, int nAW)>(0xFCF750);
	static inline auto FEF_AddPolygonToSceneOv0 = PreyFunction<CRenderObject* (CRenderer* const _this, SShaderItem& si, CRenderObject* obj, const SRenderingPassInfo& passInfo, int numPts, int ninds, SVF_P3F_C4B_T2F* & verts, SPipTangents* & tangs, uint16_t* & inds, int nAW)>(0xFCFAC0);
	static inline auto FFX_CheckOverflow = PreyFunction<void(CRenderer* const _this, int nVerts, int nInds, CRendElementBase* re, int* nNewVerts, int* nNewInds)>(0xF292F0);
	static inline auto FFX_Start = PreyFunction<void(CRenderer* const _this, CShader* ef, int nTech, CShaderResources* Res, CRendElementBase* re)>(0xF2C670);
	static inline auto FEF_AddMultipleParticlesToScene = PreyFunction<void(CRenderer* const _this, const SAddParticlesToSceneJob* jobs, uint64_t numJobs, const SRenderingPassInfo& passInfo)>(0xFF17E0);
	static inline auto FEF_SetShaderMissCallback = PreyFunction<void(CRenderer* const _this, void (*callback)(const char*))>(0xFD21D0);
	static inline auto FEF_GetShaderMissLogPath = PreyFunction<const char* (CRenderer* const _this)>(0xFD06E0);
	static inline auto FEF_GetShaderNames = PreyFunction<string* (CRenderer* const _this, int& nNumShaders)>(0xFD06F0);
	static inline auto FEF_LoadShader = PreyFunction<IShader* (CRenderer* const _this, const char* name, int flags, uint64_t nMaskGen)>(0xFD0AE0);
	static inline auto FEF_LoadShaderItem = PreyFunction<SShaderItem*(CRenderer* const _this, SShaderItem* _return_value_, const char* szName, bool bShare, int flags, SInputShaderResources* Res, uint64_t nMaskGen, const IRenderer::SLoadShaderItemArgs* pArgs)>(0xFD0B00);
	static inline auto FEF_GetRemapedShaderMaskGen = PreyFunction<uint64_t(CRenderer* const _this, const char* name, uint64_t nMaskGen, bool bFixup)>(0xFD0660);
	static inline auto FEF_GetShaderGlobalMaskGenFromString = PreyFunction<uint64_t(CRenderer* const _this, const char* szShaderName, const char* szShaderGen, uint64_t nMaskGen)>(0xFD0680);
	static inline auto FEF_GetStringFromShaderGlobalMaskGen = PreyFunction<const char* (CRenderer* const _this, const char* szShaderName, uint64_t nMaskGen)>(0xFD0780);
	static inline auto FEF_ReloadFile = PreyFunction<bool(CRenderer* const _this, const char* szFileName)>(0xFD1BD0);
	static inline auto FEF_ReloadFile_Request = PreyFunction<bool(CRenderer* const _this, const char* szFileName)>(0xFD1F60);
	static inline auto FEF_ReloadTextures = PreyFunction<void(CRenderer* const _this)>(0xFD2140);
	static inline auto FEF_GetTextureByID = PreyFunction<ITexture* (CRenderer* const _this, int Id)>(0xFD07D0);
	static inline auto FEF_GetTextureByName = PreyFunction<ITexture* (CRenderer* const _this, const char* nameTex, unsigned flags)>(0xFD07F0);
	static inline auto FEF_LoadTexture = PreyFunction<ITexture* (CRenderer* const _this, const char* nameTex, const unsigned flags)>(0xFD0B50);
	static inline auto FEF_CacheTextureAsync = PreyFunction<unsigned(CRenderer* const _this, const char* _textureName, unsigned _flags, EDeviceMemoryHeap _heapId)>(0xEE5010);
	static inline auto FEF_ReleaseCacheTexture = PreyFunction<void(CRenderer* const _this, unsigned _textureId)>(0xEE5170);
	static inline auto FEF_TryGetCacheTexture = PreyFunction<ITexture* (CRenderer* const _this, unsigned _textureId)>(0xEE5180);
	static inline auto FEF_LoadDynTexture = PreyFunction<IDynTextureSource* (CRenderer* const _this, const char* dynsourceName, bool sharedRT)>(0xFD0990);
	static inline auto FGetShaderProfile = PreyFunction<const SShaderProfile& (const CRenderer* const _this, EShaderType eST)>(0x1036550);
	static inline auto FEF_SetShaderQuality = PreyFunction<void(CRenderer* const _this, EShaderType eST, EShaderQuality eSQ)>(0xFD21E0);
	static inline auto FEF_LoadImage = PreyFunction<_smart_ptr<IImageFile>*(CRenderer* const _this, _smart_ptr<IImageFile>* _return_value_, const char* szFileName, unsigned nFlags)>(0xFD09F0);
	static inline auto FEF_CreateRE = PreyFunction<CRendElementBase* (CRenderer* const _this, EDataType edt)>(0xFCFEA0);
	static inline auto FEF_StartEf = PreyFunction<void(CRenderer* const _this, const SRenderingPassInfo& passInfo)>(0xFD2220);
	static inline auto FEF_GetObject_Temp = PreyFunction<CRenderObject* (CRenderer* const _this, int nThreadID)>(0xEC80C0);
	static inline auto FEF_DuplicateRO = PreyFunction<CRenderObject* (CRenderer* const _this, CRenderObject* pSrc, const SRenderingPassInfo& passInfo)>(0xEC7E90);
	static inline auto FEF_GetObject = PreyFunction<CRenderObject* (CRenderer* const _this)>(0xEC80B0);
	static inline auto FEF_FreeObject = PreyFunction<void(CRenderer* const _this, CRenderObject* pObj)>(0xEC7F50);
	static inline auto FEF_AddEf = PreyFunction<void(CRenderer* const _this, CRendElementBase* re, SShaderItem& pSH, CRenderObject* obj, const SRenderingPassInfo& passInfo, int nList, int nAW)>(0xEC7210);
	static inline auto FEF_InvokeShadowMapRenderJobs = PreyFunction<void(CRenderer* const _this, CRenderView* pRenderView, const int nFlags)>(0x1333E90);
	static inline auto FEF_IsFakeDLight = PreyFunction<bool(CRenderer* const _this, const CDLight* Source)>(0xFD0930);
	static inline auto FEF_ADDDlight = PreyFunction<void(CRenderer* const _this, CDLight* Source, const SRenderingPassInfo& passInfo)>(0xFCF660);
	static inline auto FEF_AddDeferredDecal = PreyFunction<bool(CRenderer* const _this, const SDeferredDecal& rDecal, const SRenderingPassInfo& passInfo)>(0xFCF740);
	static inline auto FEF_AddDeferredLight = PreyFunction<int(CRenderer* const _this, const CDLight& pLight, float fMult, const SRenderingPassInfo& passInfo, const SRendParams& _rendParams)>(0xEF9970);
	static inline auto FEF_ReleaseDeferredData = PreyFunction<void(CRenderer* const _this)>(0xEF9B70);
	static inline auto FEF_CreateInputShaderResource = PreyFunction<SInputShaderResources* (CRenderer* const _this, IRenderShaderResources* pOptionalCopyFrom)>(0xFCFE50);
	static inline auto FClearPerFrameData = PreyFunction<void(CRenderer* const _this, const SRenderingPassInfo& passInfo)>(0x1333E90);
	static inline auto FEF_GetLightAnimColor = PreyFunction<ColorF*(CRenderer* const _this, ColorF* _return_value_, uint8_t _style, uint8_t _phase, float _animSpeed)>(0xFD04C0);
	static inline auto FEF_UpdateDLight = PreyFunction<bool(CRenderer* const _this, SRenderLight* dl)>(0xFD2570);
	static inline auto FEF_CheckLightMaterial = PreyFunction<void(CRenderer* const _this, CDLight* pLight, uint16_t nRenderLightID, const SRenderingPassInfo& passInfo, const SRendParams& _rendParams)>(0xFCFC40);
	static inline auto FEF_AddWaterSimHit = PreyFunction<void(CRenderer* const _this, const Vec3& vPos, const float scale, const float strength)>(0xFCFC10);
	static inline auto FEF_DrawWaterSimHits = PreyFunction<void(CRenderer* const _this)>(0xFD0490);
	static inline auto FEF_QueryImpl = PreyFunction<void(CRenderer* const _this, ERenderQueryTypes eQuery, void* pInOut0, unsigned nInOutSize0, void* pInOut1, unsigned nInOutSize1)>(0xFD0F90);
	static inline auto FFX_SetStencilState = PreyFunction<void(CRenderer* const _this, int st, unsigned nStencRef, unsigned nStencMask, unsigned nStencWriteMask, bool bForceFullReadMask)>(0xF028D0);
	static inline auto FEF_PostEffectParamHandleFromName = PreyFunction<uint64_t(const CRenderer* const _this, const char* _szParamName)>(0xFD0CB0);
	static inline auto FEF_SetPostEffectParamOv2 = PreyFunction<void(const CRenderer* const _this, const SSetPostEffectParam& _param)>(0xFD2170);
	static inline auto FEF_SetPostEffectParamOv1 = PreyFunction<void(const CRenderer* const _this, const SSetPostEffectParamVec4& _param)>(0xFD21B0);
	static inline auto FEF_SetPostEffectParamOv0 = PreyFunction<void(const CRenderer* const _this, const SSetPostEffectParamString& _param)>(0xFD2190);
	static inline auto FEF_GetPostEffectParam = PreyFunction<float(const CRenderer* const _this, uint64_t _handle)>(0xFD05C0);
	static inline auto FEF_GetPostEffectParamVec4 = PreyFunction<Vec4*(const CRenderer* const _this, Vec4* _return_value_, uint64_t _handle)>(0xFD0600);
	static inline auto FEF_GetPostEffectParamString = PreyFunction<const char* (const CRenderer* const _this, uint64_t _handle)>(0xFD05E0);
	static inline auto FEF_IsPostEffectParam = PreyFunction<bool(const CRenderer* const _this, const char* _szParamName)>(0xFD0970);
	static inline auto FEF_ResetPostEffects = PreyFunction<void(CRenderer* const _this, bool bOnSpecChange)>(0xFD2150);
	static inline auto FEF_DisableTemporalEffects = PreyFunction<void(CRenderer* const _this)>(0xFD0480);
	static inline auto FForceGC = PreyFunction<void(CRenderer* const _this)>(0xFD3B80);
	static inline auto FCreateRenderMesh = PreyFunction<_smart_ptr<IRenderMesh>*(CRenderer* const _this, _smart_ptr<IRenderMesh>* _return_value_, const char* szType, const char* szSourceName, IRenderMesh::SInitParamerers* pInitParams, ERenderMeshType eBufType)>(0xFCEC10);
	static inline auto FCreateRenderMeshInitialized = PreyFunction<_smart_ptr<IRenderMesh>*(CRenderer* const _this, _smart_ptr<IRenderMesh>* _return_value_, const void* pVertBuffer, int nVertCount, EVertexFormat eVF, const uint16_t* pIndices, int nIndices, const PublicRenderPrimitiveType nPrimetiveType, const char* szType, const char* szSourceName, ERenderMeshType eBufType, int nMatInfoCount, int nClientTextureBindID, bool (*PrepareBufferCallback)(IRenderMesh*, bool), void* CustomData, bool bOnlyVideoBuffer, bool bPrecache, const SPipTangents* pTangents, bool bLockForThreadAcc, Vec3* pNormals)>(0xFCED40);
	static inline auto FGetMaxActiveTexturesARB = PreyFunction<int(CRenderer* const _this)>(0x1CBB0B0);
	static inline auto FGetThreadIDs = PreyFunction<void(const CRenderer* const _this, unsigned long& mainThreadID, unsigned long& renderThreadID)>(0xFD5400);
	static inline auto FSF_ConfigMask = PreyFunction<void(CRenderer* const _this, IRenderer::ESFMaskOp maskOp, unsigned stencilRef)>(0xEE6770);
	static inline auto FSF_CreateTexture = PreyFunction<int(CRenderer* const _this, int width, int height, int numMips, uint8_t* pData, ETEX_Format eTF, int flags)>(0xEE6830);
	static inline auto FSF_GetMeshMaxSize = PreyFunction<void(const CRenderer* const _this, int& numVertices, int& numIndices)>(0xEE7670);
	static inline auto FCreateTexture = PreyFunction<ITexture* (CRenderer* const _this, const char* name, int width, int height, int numMips, uint8_t* pData, ETEX_Format eTF, int flags)>(0xFCF000);
	static inline auto FSetProfileMarker = PreyFunction<void(const CRenderer* const _this, const char* label, CRenderer::ESPM mode)>(0x1333E90);
	static inline auto FPushFogVolumeContribution = PreyFunction<uint16_t(CRenderer* const _this, const ColorF& fogVolumeContrib, const SRenderingPassInfo& passInfo)>(0xEC8460);
	static inline auto FGetFogVolumeContribution = PreyFunction<void(const CRenderer* const _this, uint16_t idx, ColorF& rColor)>(0xEC8230);
	static inline auto FGetMaxTextureSize = PreyFunction<int(CRenderer* const _this)>(0xF6A170);
	static inline auto FGetShadowFrustumMGPUCache = PreyFunction<ShadowFrustumMGPUCache* (CRenderer* const _this)>(0xF6B580);
	static inline auto FGetCachedShadowsResolution = PreyFunction<const StaticArray<int,16,int>& (const CRenderer* const _this)>(0xF69CE0);
	static inline auto FSetCachedShadowsResolution = PreyFunction<void(CRenderer* const _this, const StaticArray<int,16,int>& arrResolutions)>(0xF70010);
	static inline auto FUpdateCachedShadowsLodCount = PreyFunction<void(const CRenderer* const _this, int nGsmLods)>(0xFD79A0);
	static inline auto FExecuteAsyncDIP = PreyFunction<void(CRenderer* const _this)>(0xFD3360);
	static inline auto FGetFreeChunk = PreyFunction<alloc_info_struct* (CRenderer* const _this, int bytes_count, int nBufSize, PodArray<alloc_info_struct,0>& alloc_info, const char* szSource)>(0x1071A50);
	static inline auto FReleaseChunk = PreyFunction<bool(CRenderer* const _this, int p, PodArray<alloc_info_struct,0>& alloc_info)>(0x1071FD0);
	static inline auto FGetTextureFormatName = PreyFunction<const char* (CRenderer* const _this, ETEX_Format eTF)>(0xFD53E0);
	static inline auto FGetTextureFormatDataSize = PreyFunction<int(CRenderer* const _this, int nWidth, int nHeight, int nDepth, int nMips, ETEX_Format eTF)>(0xFD53A0);
	static inline auto FSetDefaultMaterials = PreyFunction<void(CRenderer* const _this, IMaterial* pDefMat, IMaterial* pTerrainDefMat)>(0xF707D0);
	static inline auto FGetTextureSubImageData32 = PreyFunction<uint8_t* (CRenderer* const _this, uint8_t* pData, int nDataSize, int nX, int nY, int nW, int nH, CTexture* pTex)>(0x1CBB0B0);
	static inline auto FPrecacheTexture = PreyFunction<void(CRenderer* const _this, ITexture* pTP, float fMipFactor, float fTimeToReady, int Flags, int nUpdateId, int nCounter)>(0xFD6870);
	static inline auto FEF_CreateSkinningData = PreyFunction<SSkinningData* (CRenderer* const _this, unsigned nNumBones, bool bNeedJobSyncVar)>(0xFD0340);
	static inline auto FEF_CreateRemappedSkinningData = PreyFunction<SSkinningData* (CRenderer* const _this, unsigned nNumBones, SSkinningData* pSourceSkinningData, unsigned nCustomDataSize, unsigned pairGuid)>(0xFD0280);
	static inline auto FEF_GetSkinningPoolID = PreyFunction<int(CRenderer* const _this)>(0xFD0770);
	static inline auto FUpdateShaderItem = PreyFunction<void(CRenderer* const _this, SShaderItem* pShaderItem, IMaterial* pMaterial)>(0xFD79B0);
	static inline auto FForceUpdateShaderItem = PreyFunction<void(CRenderer* const _this, SShaderItem* pShaderItem, IMaterial* pMaterial)>(0xFD3BE0);
	static inline auto FRefreshShaderResourceConstants = PreyFunction<void(CRenderer* const _this, SShaderItem* pShaderItem, IMaterial* pMaterial)>(0xFD6B50);
	static inline auto FRT_UpdateShaderItem = PreyFunction<void(CRenderer* const _this, SShaderItem* pShaderItem, IMaterial* pMaterial)>(0xFD6B10);
	static inline auto FRT_RefreshShaderResourceConstants = PreyFunction<void(CRenderer* const _this, SShaderItem* pShaderItem)>(0xFD6A90);
	static inline auto FLoadShaderStartupCache = PreyFunction<bool(CRenderer* const _this)>(0xFD6630);
	static inline auto FUnloadShaderStartupCache = PreyFunction<void(CRenderer* const _this)>(0xFD7860);
	static inline auto FLoadShaderLevelCache = PreyFunction<bool(CRenderer* const _this)>(0x13B0900);
	static inline auto FUnloadShaderLevelCache = PreyFunction<void(CRenderer* const _this)>(0x1333E90);
	static inline auto FSetClearColor = PreyFunction<void(CRenderer* const _this, const Vec3& vColor)>(0xF33A50);
	static inline auto FUpdateAntiAliasingMode = PreyFunction<void(CRenderer* const _this)>(0xEBEB70);
	static inline auto FRegisterSyncWithMainListener = PreyFunction<void(CRenderer* const _this, ISyncMainWithRenderListener* pListener)>(0xFD6B60);
	static inline auto FRemoveSyncWithMainListener = PreyFunction<void(CRenderer* const _this, const ISyncMainWithRenderListener* pListener)>(0xFD6D30);
	static inline auto FCreateGraphiceDeviceConstantBuffer = PreyFunction<_smart_ptr<IGraphicsDeviceConstantBuffer>*(CRenderer* const _this, _smart_ptr<IGraphicsDeviceConstantBuffer>* _return_value_)>(0x361570);
	static inline auto FMakeMatrix = PreyFunction<void(CRenderer* const _this, const Vec3& pos, const Vec3& angles, const Vec3& scale, Matrix34* mat)>(0x1333E90);
	static inline auto FSyncMainWithRender = PreyFunction<void(CRenderer* const _this)>(0xFD7590);
	static inline auto FUpdateConstParamsPF = PreyFunction<void(CRenderer* const _this, const SRenderingPassInfo& passInfo)>(0xF08640);
	static inline auto FRT_UpdateConstParamsPF = PreyFunction<void(CRenderer* const _this, CRenderView* _pRenderView)>(0xF07CE0);
	static inline auto FGetActiveGPUCount = PreyFunction<unsigned(const CRenderer* const _this)>(0xEFB0C0);
	static inline auto FLogv = PreyFunction<void(CRenderer* const _this, const char* format, ... _arg1_)>(0xFD6640);
	static inline auto FRT_UpdateLightVolumes = PreyFunction<void(CRenderer* const _this)>(0xFD6AF0);
	static inline auto FRT_SetSkinningPoolId = PreyFunction<void(CRenderer* const _this, unsigned poolId)>(0xFD6AE0);
	static inline auto FEF_AddEf_NotVirtual = PreyFunction<void(CRenderer* const _this, CRendElementBase* re, SShaderItem& SH, CRenderObject* obj, const SRenderingPassInfo& passInfo, int nList, int nAW)>(0xEC7220);
	static inline auto FFX_StartMerging = PreyFunction<void(CRenderer* const _this)>(0xFD3500);
	static inline auto FFX_PushVP = PreyFunction<void(CRenderer* const _this)>(0xFD34C0);
	static inline auto FFX_PopVP = PreyFunction<void(CRenderer* const _this)>(0xFD3410);
	static inline auto FEF_AddRTStat = PreyFunction<void(CRenderer* const _this, CTexture* pTex, int nFlags, int nW, int nH)>(0xFFB500);
	static inline auto FFX_GetAntialiasingType = PreyFunction<eAntialiasingType(const CRenderer* const _this)>(0xFD33F0);
	static inline auto FFX_GetMSAAMode = PreyFunction<unsigned(const CRenderer* const _this)>(0x1CBB0B0);
	static inline auto FFX_SetMSAAFlagsRT = PreyFunction<void(CRenderer* const _this)>(0x1333E90);
	static inline auto FFX_ApplyShaderQuality = PreyFunction<void(CRenderer* const _this, const EShaderType eST)>(0xFD3370);
	static inline auto FIsHDRModeEnabled = PreyFunction<bool(const CRenderer* const _this)>(0xFD65F0);
	static inline auto FGetCamera = PreyFunction<const CCamera& (CRenderer* const _this)>(0xEFB0E0);
	static inline auto FGetRCamera = PreyFunction<const CRenderCamera& (CRenderer* const _this)>(0xEAFC20);
	static inline auto FGetTexturesStreamPoolSize = PreyFunction<int()>(0xFD53F0);
	static inline auto FEF_RemoveParticlesFromScene = PreyFunction<void(CRenderer* const _this)>(0xFF1AB0);
	static inline auto FPrepareParticleRenderObjects = PreyFunction<void(CRenderer* const _this, Array<SAddParticlesToSceneJob const ,int,NArray::ArrayStorage> aJobs, int nREStart, SRenderingPassInfo passInfoOriginal)>(0xFF1B00);
	static inline auto FEF_GetParticleListAndBatchFlags = PreyFunction<bool(CRenderer* const _this, unsigned& nBatchFlags, int& nList, SShaderItem& shaderItem, CRenderObject* pRO, const SRenderingPassInfo& passInfo)>(0xFF1880);
	static inline auto FFreePermanentRenderObjects = PreyFunction<void(CRenderer* const _this, int bufferId)>(0xFD3BF0);
	static inline auto FStopRendererAtFrameEnd = PreyFunction<bool(CRenderer* const _this, unsigned timeoutMilliseconds)>(0xFD74E0);
	static inline auto FResumeRendererFromFrameEnd = PreyFunction<void(CRenderer* const _this)>(0xFD6D80);
	static inline auto FSetTexturePrecaching = PreyFunction<void(CRenderer* const _this, bool stat)>(0xFD7230);
	static inline auto FGetGPUFrameTime = PreyFunction<float(CRenderer* const _this)>(0xFD5010);
	static inline auto FGetRenderTimes = PreyFunction<void(CRenderer* const _this, IRenderer::SRenderTimes& outTimes)>(0xFD5260);
	static inline auto FEnableBatchMode = PreyFunction<void(CRenderer* const _this, bool enable)>(0xFD32F0);
};
#endif // !MOONCRASH
