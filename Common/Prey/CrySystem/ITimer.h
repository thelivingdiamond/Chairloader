...
struct ITimer
{
	enum ETimer : uint8_t
	{
		...
		ETIMER_PLAYER,   //!< Guess: time used for the player in Combat Focus
		ETIMER_LAST
	};
	...
	virtual float GetTimeScale(ETimer which = ETIMER_GAME) const = 0;
	...
	virtual void SetTimeScale(ETimer which, float s, ETimeDilationChannel channel = GENERIC) = 0;
