...
struct IConsole
{
	...
	virtual void DumpCVarsToFile(const char *, unsigned int nFlagsFilter = 0) = 0;
	...
	virtual void CopyToClipboard(const char *) = 0;
	...
	virtual void AddCommand(const char* sCommand, ConsoleCommandFunc func, int nFlags = 0, const char* sHelp = nullptr) = 0;
	...
	//! \param sHelp       Help string, will be displayed when typing in console "command ?".
	virtual void AddCommand(const char* sName, const char* sScriptFunc, int nFlags = 0, const char* sHelp = nullptr) = 0;
...
struct ICVar
{
	...
	//! \return The variable's help text, can be nullptr if no help is available.
