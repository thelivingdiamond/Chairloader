...
#include <Prey/CryScriptSystem/IScriptSystem.h>
#include <Prey/CryScriptSystem/ScriptBindings/ScriptBinding.h>
#include <Prey/CryScriptSystem/ScriptSystem.cpp>
#include <Prey/CryScriptSystem/ScriptSystem.h>
#include <Prey/CrySystem/ISystem.h>
...
public:
	...
	std::vector<string> m_vecPreCached;
	SScriptFuncHandle* m_pErrorHandlerFunc;
	std::set<string, stl::less_stricmp<CryStringT<char>>> m_dqLoadedFiles;
	CScriptBindings m_stdScriptBinds;
	ISystem* m_pSystem;
	float m_fGCFreq;
	float m_lastGCTime;
	...
	virtual void GetScriptHash(const char* sPath, const char* szKey, unsigned& dwHash);
	virtual void PostInit();
	virtual void RaiseError(const char* format, ... sBuf);
	virtual void LoadScriptedSurfaceTypes(const char* sFolder, bool bReload);
	virtual void SerializeTimers(ISerialize* pSer);
	virtual void ResetTimers();
	virtual int GetStackSize();
...
{ // Size=1080 (0x438)
	...
	const char* GetKeyStr(const char* sKey, int nKey) { return FGetKeyStr(this, sKey, nKey); }
	virtual ~SRecursiveLuaDumpToFile();
...
#if 0
	const char* GetOffsetStr(int _arg0_);
	SRecursiveLuaDumpToFile(const char* _arg0_);
#endif

	static inline auto FGetKeyStr = PreyFunction<const char* (SRecursiveLuaDumpToFile* const _this, const char* sKey, int nKey)>(0xD2E2E0);
	static inline auto FOnElement = PreyFunction<void(SRecursiveLuaDumpToFile* const _this, int nLevel, const char* sKey, int nKey, ScriptAnyValue& value)>(0xD2F0B0);
	static inline auto FOnBeginTable = PreyFunction<void(SRecursiveLuaDumpToFile* const _this, int nLevel, const char* sKey, int nKey)>(0xD2F010);
	static inline auto FOnEndTable = PreyFunction<void(SRecursiveLuaDumpToFile* const _this, int nLevel)>(0xD2F4C0);
};
#endif // !MOONCRASH
