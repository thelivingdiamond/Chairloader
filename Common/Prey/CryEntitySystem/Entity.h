...
// CEntity
// Header:  CryEngine/cryentitysystem/Entity.h
// Include: Prey/CryEntitySystem/Entity.h
...
public:
	...
	using TProxyPair = std::pair<int, std::shared_ptr<IEntityProxy>>;
	using TProxyContainer = std::map<int,std::shared_ptr<IEntityProxy>,std::less<unsigned int>,stl::STLPoolAllocator<std::pair<int,std::shared_ptr<IEntityProxy> >,stl::PSyncMultiThread,0,0> >;
	using TComponents = std::set<std::shared_ptr<IComponent>>;
	...
	unsigned m_flags;
	unsigned m_flagsExtended;
	static inline auto m_szDescription = PreyGlobal<string>(0x23B8C00);
	IEntityClass* m_pClass;
	IEntityArchetype* m_pArchetype;
	Vec3 m_vPos;
	Quat m_qRotation;
	...
	unsigned m_aiObjectID;
	_smart_ptr<IMaterial> m_pMaterial;
	std::map<int,std::shared_ptr<IEntityProxy>,std::less<unsigned int>,stl::STLPoolAllocator<std::pair<int,std::shared_ptr<IEntityProxy> >,stl::PSyncMultiThread,0,0> > m_proxy;
	...
	virtual IEntityClass* GetClass() const;
	virtual IEntityArchetype* GetArchetype();
	virtual const IEntityArchetype* GetArchetype() const;
	virtual void SetFlags(unsigned flags);
	virtual unsigned GetFlags() const;
	virtual void AddFlags(unsigned flagsToAdd);
	virtual void ClearFlags(unsigned flagsToClear);
	...
	virtual bool IsParentAttachmentValid() const;
	virtual IEntity* GetAdam();
	virtual void SetWorldTM(const Matrix34& tm, int nWhyFlags);
	virtual void SetLocalTM(const Matrix34& tm, int nWhyFlags);
	virtual void SetPosRotScale(const Vec3& vPos, const Quat& qRotation, const Vec3& vScale, int nWhyFlags);
	virtual Matrix34 GetLocalTM() const;
	virtual const Matrix34& GetWorldTM() const;
	virtual void GetWorldBounds(AABB& bbox) const;
	virtual void GetLocalBounds(AABB& bbox) const;
	virtual void SetPos(const Vec3& vPos, int nWhyFlags, bool bRecalcPhyBounds, bool bForce);
	virtual const Vec3& GetPos() const;
	virtual void SetRotation(const Quat& qRotation, int nWhyFlags);
	virtual const Quat& GetRotation() const;
	virtual void SetScale(const Vec3& vScale, int nWhyFlags);
	virtual const Vec3& GetScale() const;
	virtual Vec3 GetWorldPos() const;
	virtual Ang3 GetWorldAngles() const;
	virtual Quat GetWorldRotation() const;
	virtual const Vec3& GetForwardDir() const;
	virtual void Activate(bool bActive);
	virtual bool IsActive() const;
	virtual bool IsFromPool() const;
	virtual void PrePhysicsActivate(bool bActive);
	...
	virtual const Matrix34& GetSlotLocalTM(int nSlot, bool bRelativeToParent) const;
	virtual void SetSlotLocalTM(int nSlot, const Matrix34& localTM, int nWhyFlags);
	virtual void SetSlotCameraSpacePos(int nSlot, const Vec3& cameraSpacePos);
	virtual void GetSlotCameraSpacePos(int nSlot, Vec3& cameraSpacePos) const;
	virtual bool SetParentSlot(int nParentSlot, int nChildSlot);
	virtual void SetSlotMaterial(int nSlot, IMaterial* pMaterial);
	virtual void SetSlotFlags(int nSlot, unsigned nFlags);
	...
	virtual int LoadGeomCache(int nSlot, const char* sFilename);
	virtual int SetParticleEmitter(int nSlot, IParticleEmitter* pEmitter, bool bSerialize);
	virtual int LoadParticleEmitter(int nSlot, IParticleEffect* pEffect, const SpawnParams* params, bool bPrime, bool bSerialize);
	virtual int SetBreakableGlass(int _nSlot, IArkGlass* _pGlass);
	virtual IArkGlass* GetBreakableGlass(int _nSlot);
	virtual int LoadLight(int nSlot, CDLight* pLight);
	int LoadLightImpl(int nSlot, CDLight* pLight) { return FLoadLightImpl(this, nSlot, pLight); }
	virtual bool UpdateLightClipBounds(CDLight& light);
	int LoadFogVolume(int nSlot, const SFogVolumeProperties& properties) { return FLoadFogVolume(this, nSlot, properties); }
	int FadeGlobalDensity(int nSlot, float fadeTime, float newGlobalDensity) { return FFadeGlobalDensity(this, nSlot, fadeTime, newGlobalDensity); }
	int LoadVolumeObject(int nSlot, const char* sFilename) { return FLoadVolumeObject(this, nSlot, sFilename); }
	int SetVolumeObjectMovementProperties(int nSlot, const SVolumeObjectMovementProperties& properties) { return FSetVolumeObjectMovementProperties(this, nSlot, properties); }
	virtual void InvalidateTM(int nWhyFlags, bool bRecalcPhyBounds);
	virtual void SerializeXML(XmlNodeRef& node, bool bLoading);
	virtual IEntityLink* GetEntityLinks();
	virtual IEntityLink* AddEntityLink(const char* sLinkName, unsigned entityId);
	...
	void DeallocBindings() { FDeallocBindings(this); }
	void OnRellocate(int nWhyFlags) { FOnRellocate(this, nWhyFlags); }
	void AnimationEvent(ICharacterInstance* pCharacter, const AnimEventInstance& event) { FAnimationEvent(this, pCharacter, event); }

#if 0
	IEntitySystem* GetEntitySystem() const;
	void SetLoadedFromLevelFile(const bool _arg0_);
	void Hide(bool _arg0_, EEntityEvent _arg1_, EEntityEvent _arg2_);
	CRenderProxy* GetRenderProxy() const;
	CPhysicalProxy* GetPhysicalProxy() const;
	CScriptProxy* GetScriptProxy() const;
	CEntitySystem* GetCEntitySystem() const;
	bool IsPoolControlled() const;
	bool GetUpdateStatus() const;
	const Matrix34& GetWorldTM_Fast() const;
	SProximityElement* GetProximityElement();
	static void ClearStaticData();
	void SetCloneLayerId(int _arg0_);
	int GetCloneLayerId() const;
	bool IsOnlyInOtherScene() const;
	void LogEvent(SEntityEvent& _arg0_, CTimeValue _arg1_);
	void ComputeForwardDir() const;
	bool IsScaled(float _arg0_) const;
	IAIObject* GetAIObject();
#endif

	static inline auto FCEntityOv1 = PreyFunction<void(CEntity* const _this, SEntitySpawnParams& params)>(0x91E710);
	static inline auto FInit = PreyFunction<bool(CEntity* const _this, SEntitySpawnParams& params)>(0x9220C0);
	static inline auto FPrePhysicsUpdate = PreyFunction<void(CEntity* const _this, float fFrameTime)>(0x9243C0);
	static inline auto FUpdate = PreyFunction<void(CEntity* const _this, SEntityUpdateContext& ctx)>(0x929290);
	static inline auto FShutDown = PreyFunction<void(CEntity* const _this, bool bRemoveAI, bool bRemoveProxies)>(0x928E90);
	static inline auto FGetId = PreyFunction<unsigned(const CEntity* const _this)>(0x11883D0);
	static inline auto FGetGuid = PreyFunction<uint64_t(const CEntity* const _this)>(0x598E60);
	static inline auto FGetClass = PreyFunction<IEntityClass* (const CEntity* const _this)>(0x7E3C90);
	static inline auto FGetArchetypeOv1 = PreyFunction<IEntityArchetype* (CEntity* const _this)>(0x19B580);
	static inline auto FGetArchetypeOv0 = PreyFunction<const IEntityArchetype* (const CEntity* const _this)>(0x19B580);
	static inline auto FSetFlags = PreyFunction<void(CEntity* const _this, unsigned flags)>(0x927370);
	static inline auto FGetFlags = PreyFunction<unsigned(const CEntity* const _this)>(0xD8FD10);
	static inline auto FAddFlags = PreyFunction<void(CEntity* const _this, unsigned flagsToAdd)>(0x91F010);
	static inline auto FClearFlags = PreyFunction<void(CEntity* const _this, unsigned flagsToClear)>(0x91F950);
	static inline auto FCheckFlags = PreyFunction<bool(const CEntity* const _this, unsigned flagsToCheck)>(0x91F690);
	static inline auto FSetFlagsExtended = PreyFunction<void(CEntity* const _this, unsigned _flags)>(0x927400);
	static inline auto FGetFlagsExtended = PreyFunction<unsigned(const CEntity* const _this)>(0xA0CFE0);
	static inline auto FIsGarbage = PreyFunction<bool(const CEntity* const _this)>(0x922910);
	static inline auto FIsLoadedFromLevelFile = PreyFunction<bool(const CEntity* const _this)>(0x922960);
	static inline auto FSetName = PreyFunction<void(CEntity* const _this, const char* sName)>(0x927830);
	static inline auto FGetName = PreyFunction<const char* (const CEntity* const _this)>(0x9ED890);
	static inline auto FGetEntityTextDescription = PreyFunction<const char* (const CEntity* const _this)>(0x9ED890);
	static inline auto FAttachChild = PreyFunction<void(CEntity* const _this, IEntity* pChildEntity, const SChildAttachParams& attachParams)>(0x91F240);
	static inline auto FDetachAll = PreyFunction<void(CEntity* const _this, int nDetachFlags)>(0x920410);
	static inline auto FDetachThis = PreyFunction<void(CEntity* const _this, int nDetachFlags, int nWhyFlags)>(0x920470);
	static inline auto FGetChildCount = PreyFunction<int(const CEntity* const _this)>(0x920AA0);
	static inline auto FGetChild = PreyFunction<IEntity* (const CEntity* const _this, int nIndex)>(0x920A70);
	static inline auto FEnableInheritXForm = PreyFunction<void(CEntity* const _this, bool bEnable)>(0x9206B0);
	static inline auto FGetParent = PreyFunction<IEntity* (const CEntity* const _this)>(0x920F40);
	static inline auto FGetParentAttachPointWorldTM = PreyFunction<Matrix34*(const CEntity* const _this, Matrix34* _return_value_)>(0x920F60);
	static inline auto FIsParentAttachmentValid = PreyFunction<bool(const CEntity* const _this)>(0x922B30);
	static inline auto FGetAdam = PreyFunction<IEntity* (CEntity* const _this)>(0x920970);
	static inline auto FSetWorldTM = PreyFunction<void(CEntity* const _this, const Matrix34& tm, int nWhyFlags)>(0x928820);
	static inline auto FSetLocalTM = PreyFunction<void(CEntity* const _this, const Matrix34& tm, int nWhyFlags)>(0x927480);
	static inline auto FSetPosRotScale = PreyFunction<void(CEntity* const _this, const Vec3& vPos, const Quat& qRotation, const Vec3& vScale, int nWhyFlags)>(0x927DA0);
	static inline auto FGetLocalTM = PreyFunction<Matrix34*(const CEntity* const _this, Matrix34* _return_value_)>(0x920E00);
	static inline auto FGetWorldTM = PreyFunction<const Matrix34& (const CEntity* const _this)>(0x10ECF20);
	static inline auto FGetWorldBounds = PreyFunction<void(const CEntity* const _this, AABB& bbox)>(0x9217D0);
	static inline auto FGetLocalBounds = PreyFunction<void(const CEntity* const _this, AABB& bbox)>(0x920C50);
	static inline auto FSetPos = PreyFunction<void(CEntity* const _this, const Vec3& vPos, int nWhyFlags, bool bRecalcPhyBounds, bool bForce)>(0x927D50);
	static inline auto FGetPos = PreyFunction<const Vec3& (const CEntity* const _this)>(0x10ECF30);
	static inline auto FSetRotation = PreyFunction<void(CEntity* const _this, const Quat& qRotation, int nWhyFlags)>(0x928030);
	static inline auto FGetRotation = PreyFunction<const Quat& (const CEntity* const _this)>(0x13691B0);
	static inline auto FSetScale = PreyFunction<void(CEntity* const _this, const Vec3& vScale, int nWhyFlags)>(0x928090);
	static inline auto FGetScale = PreyFunction<const Vec3& (const CEntity* const _this)>(0x1369190);
	static inline auto FGetWorldPos = PreyFunction<Vec3*(const CEntity* const _this, Vec3* _return_value_)>(0x921B10);
	static inline auto FGetWorldAngles = PreyFunction<Ang3*(const CEntity* const _this, Ang3* _return_value_)>(0x921660);
	static inline auto FGetWorldRotation = PreyFunction<Quat*(const CEntity* const _this, Quat* _return_value_)>(0x921B30);
	static inline auto FGetForwardDir = PreyFunction<const Vec3& (const CEntity* const _this)>(0x920AC0);
	static inline auto FActivate = PreyFunction<void(CEntity* const _this, bool bActive)>(0x91EE60);
	static inline auto FIsActive = PreyFunction<bool(const CEntity* const _this)>(0x9228F0);
	static inline auto FIsFromPool = PreyFunction<bool(const CEntity* const _this)>(0x922900);
	static inline auto FPrePhysicsActivate = PreyFunction<void(CEntity* const _this, bool bActive)>(0x9243A0);
	static inline auto FIsPrePhysicsActive = PreyFunction<bool(CEntity* const _this)>(0x922B90);
	static inline auto FSerialize = PreyFunction<void(CEntity* const _this, TSerialize ser, int nFlags)>(0x925860);
	static inline auto FSendEvent = PreyFunction<bool(CEntity* const _this, SEntityEvent& event)>(0x925210);
	static inline auto FSetTimer = PreyFunction<void(CEntity* const _this, int nTimerId, int nMilliSeconds)>(0x9285B0);
	static inline auto FKillTimer = PreyFunction<void(CEntity* const _this, int nTimerId)>(0x922C40);
	static inline auto FHideOv1 = PreyFunction<void(CEntity* const _this, bool bHide)>(0x921FA0);
	static inline auto FIsHidden = PreyFunction<bool(const CEntity* const _this)>(0x922920);
	static inline auto FInvisible = PreyFunction<void(CEntity* const _this, bool bInvisible)>(0x9227F0);
	static inline auto FIsInvisible = PreyFunction<bool(const CEntity* const _this)>(0x922940);
	static inline auto FGetAI = PreyFunction<IAIObject* (CEntity* const _this)>(0x920930);
	static inline auto FHasAI = PreyFunction<bool(const CEntity* const _this)>(0x921F80);
	static inline auto FGetAIObjectID = PreyFunction<unsigned(const CEntity* const _this)>(0x199B520);
	static inline auto FSetAIObjectID = PreyFunction<void(CEntity* const _this, unsigned id)>(0x927030);
	static inline auto FRegisterInAISystem = PreyFunction<bool(CEntity* const _this, const AIObjectParams& params)>(0x924700);
	static inline auto FUpdateAIObject = PreyFunction<void(CEntity* const _this)>(0x929450);
	static inline auto FSetUpdatePolicy = PreyFunction<void(CEntity* const _this, EEntityUpdatePolicy eUpdatePolicy)>(0x928620);
	static inline auto FGetUpdatePolicy = PreyFunction<EEntityUpdatePolicy(const CEntity* const _this)>(0x921650);
	static inline auto FGetProxy = PreyFunction<IEntityProxy* (const CEntity* const _this, EEntityProxy proxy)>(0x921160);
	static inline auto FSetProxy = PreyFunction<void(CEntity* const _this, EEntityProxy proxy, std::shared_ptr<IEntityProxy> pProxy)>(0x927EF0);
	static inline auto FCreateProxy = PreyFunction<std::shared_ptr<IEntityProxy>*(CEntity* const _this, std::shared_ptr<IEntityProxy>* _return_value_, EEntityProxy proxy)>(0x91F960);
	static inline auto FRegisterComponent = PreyFunction<void(CEntity* const _this, std::shared_ptr<IComponent> pComponentPtr, const int flagsOriginal)>(0x9245A0);
	static inline auto FPhysicalize = PreyFunction<void(CEntity* const _this, SEntityPhysicalizeParams& params)>(0x924210);
	static inline auto FEnablePhysics = PreyFunction<void(CEntity* const _this, bool enable)>(0x9206D0);
	static inline auto FGetPhysics = PreyFunction<IPhysicalEntity* (const CEntity* const _this)>(0x9210C0);
	static inline auto FPhysicalizeSlot = PreyFunction<int(CEntity* const _this, int slot, SEntityPhysicalizeParams& params)>(0x924340);
	static inline auto FUnphysicalizeSlot = PreyFunction<void(CEntity* const _this, int slot)>(0x929240);
	static inline auto FUpdateSlotPhysics = PreyFunction<void(CEntity* const _this, int slot)>(0x9295F0);
	static inline auto FSetPhysicsState = PreyFunction<void(CEntity* const _this, XmlNodeRef& physicsState)>(0x927A70);
	static inline auto FHasPhysicsState = PreyFunction<bool(const CEntity* const _this)>(0x921F90);
	static inline auto FSetMaterial = PreyFunction<void(CEntity* const _this, IMaterial* pMaterial)>(0x9277A0);
	static inline auto FGetMaterial = PreyFunction<IMaterial* (CEntity* const _this)>(0x11C8520);
	static inline auto FIsSlotValid = PreyFunction<bool(const CEntity* const _this, int nSlot)>(0x922BA0);
	static inline auto FFreeSlot = PreyFunction<void(CEntity* const _this, int nSlot)>(0x9208D0);
	static inline auto FGetSlotCount = PreyFunction<int(const CEntity* const _this)>(0x921410);
	static inline auto FGetSlotInfo = PreyFunction<bool(const CEntity* const _this, int nSlot, SEntitySlotInfo& slotInfo)>(0x9214D0);
	static inline auto FGetSlotWorldTM = PreyFunction<const Matrix34& (const CEntity* const _this, int nSlot)>(0x921590);
	static inline auto FGetSlotLocalTM = PreyFunction<const Matrix34& (const CEntity* const _this, int nSlot, bool bRelativeToParent)>(0x921530);
	static inline auto FSetSlotLocalTM = PreyFunction<void(CEntity* const _this, int nSlot, const Matrix34& localTM, int nWhyFlags)>(0x9282F0);
	static inline auto FSetSlotCameraSpacePos = PreyFunction<void(CEntity* const _this, int nSlot, const Vec3& cameraSpacePos)>(0x928230);
	static inline auto FGetSlotCameraSpacePos = PreyFunction<void(const CEntity* const _this, int nSlot, Vec3& cameraSpacePos)>(0x921380);
	static inline auto FSetParentSlot = PreyFunction<bool(CEntity* const _this, int nParentSlot, int nChildSlot)>(0x9278C0);
	static inline auto FSetSlotMaterial = PreyFunction<void(CEntity* const _this, int nSlot, IMaterial* pMaterial)>(0x928350);
	static inline auto FSetSlotFlags = PreyFunction<void(CEntity* const _this, int nSlot, unsigned nFlags)>(0x928290);
	static inline auto FGetSlotFlags = PreyFunction<unsigned(const CEntity* const _this, int nSlot)>(0x921470);
	static inline auto FShouldUpdateCharacter = PreyFunction<bool(const CEntity* const _this, int nSlot)>(0x13B0900);
	static inline auto FGetCharacter = PreyFunction<ICharacterInstance* (CEntity* const _this, int nSlot)>(0x920A10);
	static inline auto FSetCharacter = PreyFunction<int(CEntity* const _this, ICharacterInstance* pCharacter, int nSlot)>(0x927170);
	static inline auto FGetStatObj = PreyFunction<IStatObj* (CEntity* const _this, int nSlot)>(0x9215F0);
	static inline auto FSetStatObj = PreyFunction<int(CEntity* const _this, IStatObj* pStatObj, int nSlot, bool bUpdatePhysics, float mass)>(0x9283C0);
	static inline auto FGetParticleEmitter = PreyFunction<IParticleEmitter* (CEntity* const _this, int nSlot)>(0x921060);
	static inline auto FGetGeomCacheRenderNode = PreyFunction<IGeomCacheRenderNode* (CEntity* const _this, int nSlot)>(0x920BF0);
	static inline auto FMoveSlot = PreyFunction<void(CEntity* const _this, IEntity* targetIEnt, int nSlot)>(0x923A00);
	static inline auto FLoadGeometry = PreyFunction<int(CEntity* const _this, int nSlot, const char* sFilename, const char* sGeomName, int nLoadFlags)>(0x923200);
	static inline auto FLoadCharacter = PreyFunction<int(CEntity* const _this, int nSlot, const char* sFilename, int nLoadFlags)>(0x922C60);
	static inline auto FLoadGeomCache = PreyFunction<int(CEntity* const _this, int nSlot, const char* sFilename)>(0x9230C0);
	static inline auto FSetParticleEmitter = PreyFunction<int(CEntity* const _this, int nSlot, IParticleEmitter* pEmitter, bool bSerialize)>(0x927920);
	static inline auto FLoadParticleEmitter = PreyFunction<int(CEntity* const _this, int nSlot, IParticleEffect* pEffect, const SpawnParams* params, bool bPrime, bool bSerialize)>(0x923770);
	static inline auto FSetBreakableGlass = PreyFunction<int(CEntity* const _this, int _nSlot, IArkGlass* _pGlass)>(0x927040);
	static inline auto FGetBreakableGlass = PreyFunction<IArkGlass* (CEntity* const _this, int _nSlot)>(0x9209B0);
	static inline auto FLoadLight = PreyFunction<int(CEntity* const _this, int nSlot, CDLight* pLight)>(0x9234B0);
	static inline auto FLoadLightImpl = PreyFunction<int(CEntity* const _this, int nSlot, CDLight* pLight)>(0x923610);
	static inline auto FUpdateLightClipBounds = PreyFunction<bool(CEntity* const _this, CDLight& light)>(0x9294F0);
	static inline auto FLoadFogVolume = PreyFunction<int(CEntity* const _this, int nSlot, const SFogVolumeProperties& properties)>(0x922F90);
	static inline auto FFadeGlobalDensity = PreyFunction<int(CEntity* const _this, int nSlot, float fadeTime, float newGlobalDensity)>(0x920780);
	static inline auto FLoadVolumeObject = PreyFunction<int(CEntity* const _this, int nSlot, const char* sFilename)>(0x9238D0);
	static inline auto FSetVolumeObjectMovementProperties = PreyFunction<int(CEntity* const _this, int nSlot, const SVolumeObjectMovementProperties& properties)>(0x9286F0);
	static inline auto FInvalidateTM = PreyFunction<void(CEntity* const _this, int nWhyFlags, bool bRecalcPhyBounds)>(0x922280);
	static inline auto FSerializeXML = PreyFunction<void(CEntity* const _this, XmlNodeRef& node, bool bLoading)>(0x926DE0);
	static inline auto FGetEntityLinks = PreyFunction<IEntityLink* (CEntity* const _this)>(0x105410);
	static inline auto FAddEntityLink = PreyFunction<IEntityLink* (CEntity* const _this, const char* sLinkName, unsigned entityId)>(0x91EF10);
	static inline auto FRemoveEntityLink = PreyFunction<void(CEntity* const _this, IEntityLink* pLink)>(0x925140);
	static inline auto FRemoveAllEntityLinks = PreyFunction<void(CEntity* const _this)>(0x925090);
	static inline auto FSetSlotAlpha = PreyFunction<void(CEntity* const _this, int _nSlot, float _alpha)>(0x9280E0);
	static inline auto FUnmapAttachedChild = PreyFunction<IEntity* (CEntity* const _this, int& partId)>(0x986240);
	static inline auto FGetMemoryUsage = PreyFunction<void(const CEntity* const _this, ICrySizer* pSizer)>(0x920E20);
	static inline auto FIsInitialized = PreyFunction<bool(const CEntity* const _this)>(0x922930);
	static inline auto FDebugDraw = PreyFunction<void(CEntity* const _this, const SGeometryDebugDrawInfo& info)>(0x9203A0);
	static inline auto FGetSlot = PreyFunction<CEntityObject* (const CEntity* const _this, int nSlot)>(0x9212F0);
	static inline auto FGetSignature = PreyFunction<bool(CEntity* const _this, TSerialize& signature)>(0x9211C0);
	static inline auto FSerializeXML_ExceptScriptProxy = PreyFunction<void(CEntity* const _this, XmlNodeRef& node, bool bLoading)>(0x926F00);
	static inline auto FReloadEntity = PreyFunction<bool(CEntity* const _this, SEntityLoadParams& loadParams)>(0x924A70);
	static inline auto FSetPoolControl = PreyFunction<void(CEntity* const _this, bool bSet)>(0x927D00);
	static inline auto FActivateForNumUpdates = PreyFunction<void(CEntity* const _this, int numUpdates)>(0x91EE80);
	static inline auto FSetUpdateStatus = PreyFunction<void(CEntity* const _this)>(0x928640);
	static inline auto FCalcLocalTM = PreyFunction<void(const CEntity* const _this, Matrix34& tm)>(0x91F4F0);
	static inline auto FIncKeepAliveCounter = PreyFunction<void(CEntity* const _this)>(0x9220B0);
	static inline auto FDecKeepAliveCounter = PreyFunction<void(CEntity* const _this)>(0x920400);
	static inline auto FResetKeepAliveCounter = PreyFunction<void(CEntity* const _this)>(0x925200);
	static inline auto FIsKeptAlive = PreyFunction<bool(const CEntity* const _this)>(0x922950);
	static inline auto FCheckMaterialFlags = PreyFunction<void(CEntity* const _this)>(0x91F6A0);
	static inline auto FGetDisplayName = PreyFunction<const char* (const CEntity* const _this)>(0x1834030);
	static inline auto FSetDisplayName = PreyFunction<void(CEntity* const _this, const char* _pName)>(0x927340);
	static inline auto FGetSceneMask = PreyFunction<uint8_t(const CEntity* const _this)>(0x9211B0);
	static inline auto FIsStatic = PreyFunction<bool(const CEntity* const _this)>(0x922C30);
	static inline auto FAllocBindings = PreyFunction<void(CEntity* const _this)>(0x91F020);
	static inline auto FDeallocBindings = PreyFunction<void(CEntity* const _this)>(0x920310);
	static inline auto FOnRellocate = PreyFunction<void(CEntity* const _this, int nWhyFlags)>(0x923F30);
	static inline auto FAnimationEvent = PreyFunction<void(CEntity* const _this, ICharacterInstance* pCharacter, const AnimEventInstance& event)>(0x91F080);
};
#endif // !MOONCRASH
