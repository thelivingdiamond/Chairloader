...
void InitTree(void)  /* initialize trees */
{
	...
	for (i = N_SIZE + 1; i <= N_SIZE + 256; i++) rson[i] = NIL;
	for (i = 0; i < N_SIZE; i++) dad[i] = NIL;
}
	   ...
	   Note r plays double role, as tree node and position in buffer. */
{
	...
	cmp = 1;  key = &text_buf[r];  p = N_SIZE + 1 + key[0];
...
int Encodem(unsigned char *getcmP, unsigned char *putcmP, int Size)
{
  ...
  for (len = 0; len < F_UPPER && (c = getcm(Count, Size, getcmP)) != EOF; len++)
	...
	for (i = 1; i <= F_UPPER; i++) InsertNode(r - i, text_buf);  /* Insert the F_UPPER strings,
