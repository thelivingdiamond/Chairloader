...
void SkipComments(char** buf, bool bSkipWhiteSpace)
{
	...
	while (n = IsComment(buf))
	{
		...
		case 3:
			...
			if (!(*buf))
			{
				gEnv->pLog->Log("Warning: Comment lines aren't closed\n");
				break;
			}
			if (bSkipWhiteSpace)
				SkipCharacters(buf, kWhiteSpace);
...
bool fxIgnorePreprBlock(char** buf)
{
	...
	{
		assert(0);
		CryWarning("Couldn't find #endif directive for associated #ifdef");
		return false;
	}
...
start:
	...
	if (b[0] == '#')
	{
		...
		else if (!strncmp(b, "#else", 5))
		{
			...
			if (sfxIFDef[nLevel] == true)
			{
				...
				{
					assert(0);
					CryWarning("#else or #elif after #else");
					return -1;
				}
			}
		}
		...
		else if (!strncmp(b, "#endif", 6))
		{
			...
			{
				assert(0);
				CryWarning("#endif without #ifdef");
				return -1;
			}
	...
	if (tokens->id == 0)
	{
		...
		gEnv->pLog->Log("Warning: Found token '%s' which was not one of the list (Skipping).\n", pp);
		while (ptokens->id != 0)
		{
			gEnv->pLog->Log("    %s\n", ptokens->token);
			ptokens++;
