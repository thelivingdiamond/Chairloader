...
static pugi::xml_document ExportExcelXmlInternal(const ExcelTable& excelTable, bool addWatermark)
{
    ...
    // Columns
    for (size_t i = 0; i < excelTable.columnNames.size(); i++)
    {
    ...
    // Column names row
    {
        ...
        for (const std::string& colName : excelTable.columnNames)
        {
    ...
    if (addWatermark && excelTable.columnNames.size() >= 2)
    {
        // Find first non-key column
        // It must exist due to the condition above
        size_t watermarkCol = 0;
        while (watermarkCol == excelTable.keyColumnIdx)
        {
            watermarkCol++;
            assert(watermarkCol < excelTable.columnNames.size());
        }
    ...
    // Rows
    for (const ExcelTable::Row& rowData : excelTable.rows)
    {
        pugi::xml_node row = table.append_child("Row");

        // Cells
        assert(rowData.size() == excelTable.columnNames.size());
...
pugi::xml_document ExcelTable::ExportExcelXml() const
{
...
{
    return ExportExcelXmlInternal(m_CurrentTable, true);
}
