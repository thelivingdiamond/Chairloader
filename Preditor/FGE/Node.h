...
struct PrototypeNode
{
    ...
    bool operator==(const PrototypeNode& other) const { return Class == other.Class; }
    // == operator based on std::string
    bool operator==(const std::string& other) const { return Class == other; }
    void setCategory();
};
...
struct Node
{
    ...
    Node(std::string name, std::shared_ptr<PrototypeNode> proto, ImVec2 pos, int64_t id,
         std::map<std::string, std::string>& defaultInputs, IChairFlowgraph* flowgraph)
        : Name(name)
        , Pos(pos)
        , ParentFlowgraph(flowgraph)
    {
...
//     float textSize;
//     ImVec4 textColor;
//     commentBox(std::string name, PrototypeNode &proto, ImVec2 pos, int64_t id, std::string comment, float textsize =
//     1, ImVec4 textcolor = {1.0,1.0,1.0,1.0})
